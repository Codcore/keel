...
1. **`keel skills`** і породжені файли для Claude, Cursor та `AGENTS.md`.
   Чекає на звірку схем із документацією (див. відкрите).
...
- **Що приходить хукові на вхід.** Імена подій і форма відповіді звірені, а от
  чим саме кожен інструмент називає шлях файлу у вхідному JSON — ні. Хук, який
  не знайшов шляху, мовчки пропустить запис: точно та порода помилки, від якої
  Keel і будується.
