...
- **Схеми конфігів агентів** — `.cursor/rules`, `.cursor/hooks.json`,
  `.codex/hooks.json`, `.claude/settings.json`. Їх треба звірити з документацією,
  перш ніж `keel skills` почне їх породжувати: породжене неправильно гірше за
  не породжене взагалі, бо мовчить.
