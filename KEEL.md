...
keel/contracts/
keel/decisions/
keel/PRINCIPLES.md              копія
keel/QUALITY.md                 копія
AGENTS.md                       породжений
...
`AGENTS.md` — **не переказ принципів**, а вказівник: де питати стан, що запускати
перед коммітом, де лежать принципи й розрізи. Він про цей проєкт; принципи — про
метод, і тому лежать окремо.
...
| `SessionStart` | Claude Code | виводить `keel next` у контекст на початку сесії |
...
Два останні хуки працюють лише в Claude. Вони роблять зворотний звʼязок швидшим,
але **нічого не тримають**: те саме ловить `pre-commit` у будь-якому агенті.
Покласти туди силу означало б, що в Cursor усе розсиплеться.
