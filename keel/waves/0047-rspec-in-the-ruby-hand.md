...
**Після рецензії (§9.9) — і це змінило текст сценарію 2, тож його
редакцію в тегу оновлено свідомо (§7.5):** правило зʼєднання виявилось
не тим, що я зміряв на трьох формах: rspec кладе `#…`/`::…`/`.…` без
пробілу **лише після константи-модуля** (`description_separator`), а
імʼя прикладу — завжди з пробілом; і стек груп мусить рахувати
ruby-ві `end` без `do` (`def`, `if`, `case`, heredoc). Обидва — важкі
знахідки R-1 і R-2; форми зіграно наново з пробою проти справжнього
`--dry-run` на 19 іменах.
...
пробіл — без пробілу лише після константи-модуля перед `#…`, `::…`
чи `.…`, і імʼя прикладу завжди з пробілом, — константа за іменем,
метадані після імені — не імʼя, — і саме за цим описом приклад
знаходиться в `--dry-run`; ruby між групами (`def`, `if`, `case`,
heredoc, коментар після `do`) груп не закриває. Тег над прикладом без
імені (`it { … }`, `it do … end`), тег усередині `shared_examples`, імʼя
з `#{…}` і група, названа не літералом, — відмови вголос, кожна називає
котру.
