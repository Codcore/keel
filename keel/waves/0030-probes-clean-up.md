...
decisions:
  ...
  reliability.recoverability: "тримає a-sandbox-does-not-outlive-its-test: пісочниця впалого тесту ЛИШАЄТЬСЯ і КАЖЕ ПРО СЕБЕ в stderr — зміряно на червоному бігу: дві теки, два рядки «kept for inspection» (до виправлення R-1 той самий червоний біг лишав дві теки мовчки). Прибирання, яке зʼїдає доказ, гірше за протікання"
  ...
  safety.operational-constraints: "тримає a-sandbox-does-not-outlive-its-test (виправлено за R-7 рецензії: тіло сценарію обіцяло «жодного іншого шляху вона не приймає», а не судило цього ніщо — рука, навчена брати шлях від того, хто кличе, лишала батарею зеленою). Тепер проба вимагає, щоб усі пісочниці висіли під одним коренем, який рука обирає сама, і щоб імʼя несло pid цього процесу. Це та межа, яку рецензент 0026 переступив руками, коли знищив 10128 чужих тек"
...
Рядок BACKLOG про пісочниці, і він перестав бути боргом. (Перший
рядок таблиці — інший, `keel close` із власним `target`; перша
редакція цього абзацу казала «перший», і рецензент 0030 R-12 це
спіймав.) Під час хвилі 0029
...
Три межі, названі вголос за R-10 і R-13 рецензії. **Перша:** `Drop`
не біжить, коли процес убито (`SIGKILL`, `abort`, подвійна паніка) —
зміряно рецензентом: убитий бінарник лишив одну теку. **Друга:**
рука прибирає теку **на вході** теж, і це єдина дорога, якою вона
може знищити навмисно збережений доказ — якщо pid обернеться і
збіжиться з тим, чия пісочниця лежить збережена. Лишено свідомо: без
цього два тести з тим самим імʼям в одному бінарнику успадкували б
чуже сміття. **Третя:** рука прибирає **лише те, що створила сама** —
...
іншого шляху вона не приймає. **Жоден файл проб не робить теки в спільному тимчасовому
каталозі власними руками** — і судиться саме це: проба шукає в
кожному `tests/*.rs` не імʼя `sandbox`, а **сам виклик стандартної
бібліотеки**, яким тека й народжується. Перша редакція сторожувала
імʼя, і через це не бачила ані `speak_test.rs`, що робив теки рукою
`bare()` і прибирав їх останнім рядком тіла, ані тієї самої течі під
будь-яким іншим імʼям (R-1 і R-2 рецензії).
