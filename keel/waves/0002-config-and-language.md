...
decisions:
  ...
  maintainability.modifiability: "свідомо без тесту: додати мову = .ftl-файл плюс рядок у словнику мов і гілка вибору; додати поле конфіга = рядок у Raw і в Config — міра мала й видима, тримається будовою"
  ...
  flexibility.replaceability: "свідомо без тесту: Fluent замінюваний лише разом із форматом .ftl-файлів — ціна вибору, названа тут вголос; заболить хіба з новим форматом перекладів"
...
`[trust]`, `[generated]`); невідоме поле чи кривий тип — відмова з
іменем поля і рядком, не мовчазний пропуск; а `lang` поза набором
вшитих мов — відмова, що називає наявні («мови pl у цьому релізі
нема: є en, uk»).

## scenario: missing-config-defaults

**Дано** проєкт без `keel.toml` — і проєкт, де `keel.toml` є, але
поля `lang` у ньому нема,
**коли** біжить `keel check`,
**тоді** інструмент працює з типовими значеннями (`lang = "en"`) і
звіт каже про це вголос у обох випадках — типове значення не видає
себе за прочитане ні для файлу, ні для окремого поля.
...
Разом їде `keel.toml` цього репозиторію з `lang = "uk"` —
самонаведення: перший проєкт нового конфіга — ми самі, і без нього
власний звіт інструмента перемкнувся б на англійську мовчки.

Застереження: обидва вшиті .ftl-файли читаються тестом на збірці
bundle-ів — зламаний переклад не доїде до релізу мовчки. Зламаний
`keel.toml` у CLI — відмова команди (вихід 2) англійською: мови
проєкту в цей момент ще не знаємо, і вгадувати її з битого файлу не
будемо.
...
Виправлення за знахідками свіжого рецензента (§9.9). З-1: типове
значення поля `lang` у присутньому keel.toml тепер назване вголос
окремим рядком звіту — сценарій missing-config-defaults доріс цим
випадком. З-3: дві причини в decisions уточнені проти живого коду.
З-4: дошиті assert-и — відмова на кривий тип конфіга тримає імʼя поля
і рядок; англійська гілка output-follows-lang тримає і відмову
документа, не лише зелений звіт.

Відмова вголос (З-2): chore-трансформа english-comments зачепила,
крім коментарів, три рядкові літерали, один з яких видимий
користувачу — список полів у підказці про невідоме поле («або» →
«or»). Це ширше за її оголошене «поведінка не змінюється»; історію не
переписуємо, зміну визнано тут, і надалі рядкові літерали входять у
текст chore явно.
