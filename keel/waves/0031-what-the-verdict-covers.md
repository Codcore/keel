...
decisions:
  ...
  compatibility.interoperability: "свідомо без тесту: git і df — те, що є всюди, де є git. (Рядок казав «ls-remote», якого в коді нема: три сусідні рядки я під цю зміну переписав, цей проґавив — R-16 рецензії.)"
  ...
  interaction.operability: "тримає the-closing-court-names-its-price: ціну сказано ПЕРЕД тим, як її платити — і тепер це правда. Перша редакція друкувала ціну у звіті, який виходить лише разом із рештою: рецензент зміряв — увесь вивід, ціну включно, зʼявився на 101-й секунді, тобто коли target уже збудовано (R-1). Тепер рядок іде в stderr до батареї"
  ...
  maintainability.testability: "тримає обидва сценарії, і це виправлення R-2 і R-3 рецензії: перша редакція ЗАЯВЛЯЛА, що проби роблять обрізаний клон, а жодна не робила — мутант, що знімав межу shallow, лишав батарею зеленою; те саме з судом диска. Тепер проба клонує з --depth 1 через file:// (локальний клон бере всю історію, хоч би яку глибину просили) і жене відмову по диску власним df на PATH. Обидва мутанти рецензента переграно — обидва ловляться"
  ...
  safety.risk-identification: "свідомо без окремої роботи: загроза названа ЗМІРЯНИМ числом — одне закриття лишає 1,26 ГіБ (зміряв рецензент). Перша редакція спиралася на 3,3 ГБ у tool/target, а то накопичена тека розробника (debug+release+doc), а не ціна одного закриття, і з неї я взяв поріг 4 ГіБ — утричі більший за потрібне, через що суд відмовляв би там, де місця досить (R-5). Поріг тепер 2 ГіБ"
  safety.fail-safe: "тримає the-closing-court-names-its-price: відмова на вході лишає диск таким, яким був"
  safety.hazard-warning: "тримає the-closing-court-names-its-price: попередження йде ДО витрати — виправлено за R-1, де воно йшло після неї"
...
**Дано** `keel close`,
**коли** його кличуть,
**тоді** він каже **перед** роботою — і справді перед, не разом зі
звітом наприкінці, — що збудує власний `target`, **де саме** (теку
бере в адаптера, а не вгадує) і чому (успадкований кеш зсуває вироки — §6.7), а **коли місця
...
left on device». Після роботи він каже, скільки той `target` важить — з десятою
часткою, бо 1,26 ГіБ, показані як «1», ховали б рівно те число,
заради якого хвиля є.
...
Один файл, оголошений у плані, **знято зі списку під час роботи**, і
це звуження названо тут, а не сховано. `scope.rs` теж був знятий —
йому нічого було додавати, бо питання ставляться його ж рукою
`git_at`, — але **рецензія повернула його** (R-14): щоб не тримати
другої руки для git, `scope::git_line` стала `pub(crate)`. Суд scope
спіймав мене на цьому рівно так, як мав.
