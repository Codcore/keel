...
`version.rs`: над розбіжним піном перша порада — дорога релізу, коли
пін має форму версії; `KEEL_REF` — друга, з тим, чим вона є. Межа, яку
хвиля 0056 вже вписала в `version-pin-hand-border`, коротшає рівно на
те, що переїхало у поради. Контракт `tool-version.md` і проба
`pin_hand_test.rs` кажуть новий порядок.
