...
transforms:
  hooks-in-both-tongues:
    implements:
      - hook-speaks-the-next-step
    contracts: [tool-generated@2d09a4, tool-next@173ac6, tool-config@0e4d22]
    files:
...
decisions:
  ...
  safety.fail-safe: "тримає hook-speaks-the-next-step: за будь-якої непевности не пишеться нічого, а слово каже, що вставити рукою. І друге, зміряне під час роботи: hook, який відмовляє, не має права виходити двійкою — у Cursor це «заблокувати дію», — тож відмова їде в формі агента із зеленим виходом; знайдено власним міром до рецензії, у стані одразу після keel init, де хвиль ще нема"
...
відмова, що називає відомі. **Hook, що говорить, говорить завжди**: у проєкті,
де кроку сказати не можна — теки `keel/` ще нема (стан одразу після
`keel init`) або `keel.toml` битий, — `--for` віддає **валідний
JSON із тією ж відмовою** всередині і **зелений вихід**, бо в Cursor
код 2 означає «заблокувати дію»; сам `keel next` без прапорця своєї
поведінки не міняє. **Власний файл людини не міняється
