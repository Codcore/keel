...
transforms:
  hooks-in-both-tongues:
    implements:
      - hook-speaks-the-next-step
    contracts: [tool-generated@99840d, tool-next@173ac6, tool-config@0e4d22]
    files:
...
decisions:
  ...
  interaction.user-assistance: "тримає hook-speaks-the-next-step, і виправлено за R-1 рецензії — найважчою знахідкою хвилі: перша редакція подавала ЦІЛИЙ документ як «записи», і рецензент зміряв, що така порада, виконана буквально, дає невалідний JSON двома дорогами з трьох, а третьою тихо зʼїдає власні hook-и людини разом зі сторожем виконання команд. Рука не зрушила байта — порада зрушила все. Тепер слово несе САМІ ЗАПИСИ і називає ключ, під який вони йдуть, а проба виконує пораду і вимагає, щоб хуки людини лишились цілими"
  ...
  interaction.self-descriptiveness: "виправлено двічі. Перша редакція обіцяла неправду: hook-конфіги — ЄДИНІ артефакти, що НЕ можуть сказати про себе «згенеровано», бо JSON коментаря не має. Друга спиралась на «Cursor у своїй схемі має additionalProperties: false» — і це знято за R-5 рецензії: живого джерела в мене нема, а єдине, де воно є, — той самий npm-пакет cursor-hooks@1.1.6, що вже раз мене підвів, і який до того ж вважає невалідним наш власний файл (у ньому нема sessionStart). Лишається правда з живих джерел: JSON не має коментаря, а Claude Code на невідомий ключ при пакуванні скіла дає тверду помилку — тож ключів понад документовані ми не вигадуємо. Каже інструмент: рядок рами і запис у [generated]"
  ...
  maintainability.testability: "виправлено за R-7 рецензії, бо продавалось сильнішим, ніж є: claude plugin validate у пробі доводить рівно те, що тека .claude/skills існує і її шапка парситься, — на всьому, що цей реліз здатен породити, він почервоніти не може (name і description — сталі літерали). Справжній суддя форми скіла — YAML-парсер проби 0024, і він строгіший. Валідатор лишається як soft-суддя проти майбутніх змін шаблона і як виконання слова оператора («використовуй їхню лібу»), але судової сили він не додав, і тепер це сказано, а не змовчано"
  flexibility.adaptability: "не застосовується: hook не залежить від адаптера мови"
  flexibility.scalability: "не застосовується"
  flexibility.installability: "свідомо без тесту, але з межею, названою за R-10 рецензії: keel init ставить hook-КОНФІГ, а не сам бінарник. Команда всередині нього кличе keel з PATH — де його нема, hook мовчить (порожній stdout, не сміття: fail-safe тримається). Це межа щабля роздачі (~/.keel/versions/), і вона тут названа, а не змовчана. Друга половина тієї ж знахідки полагоджена коммітом «a hook that speaks, speaks always»: у свіжому проєкті, де adapter ще закоментований, hook тепер віддає валідну відмову, а не порожнечу з виходом 2"
...
наш — не пише **жодного байта**, а каже, які записи і **під який
ключ** додати; трьома словами на три стани (порожній файл, чужий
файл, наш файл, правлений рукою — останній називає ще й шлях назад).
...
жодним байтом**: замість запису рядок каже, **під який ключ** ідуть
записи, і самі записи стоять **байт-у-байт усередині** того, що ліг
би у файл. І порада мусить працювати: проба **виконує** її над файлом
людини, де вже стоять свій `sessionStart` і сторож
`beforeShellExecution`, і вимагає, щоб після злиття вийшов валідний
JSON, у якому цілі **і їхні хуки, і наш**. Три стани — три слова:
порожній файл (цілий документ, бо зливати нема з чим), чужий файл
(записи і ключ), наш файл, правлений рукою (записи плюс шлях назад). Наш файл, виправлений
