...
transforms:
  ...
  every-assert-can-fail:
    chore: "суд над власною батареєю, а не обіцянка про поведінку keel: перевіряє, чи кожен асерт узагалі може впасти. Спершу я написав це сценарієм — і НОВИЙ суд цієї ж хвилі мене на цьому спіймав: сценарій без комміту red:. Він мав рацію, і послаблювати його я не став; натомість назвав роботу тим, чим вона є. Червоне народження (§6.3) має сенс для обіцянки про поведінку інструмента; для суду над нашими ж пробами стану «падає до роботи» не існує, коли клас, який він стереже, зараз чистий"
    files:
...
decisions:
  maintainability.testability: "свідомо не робимо сценарієм: суд над мертвими асертами їде chore-трансформою (every-assert-can-fail), бо це перевірка над НАШОЮ батареєю, а не обіцянка про поведінку keel. Спершу я написав це сценарієм — і новий суд цієї ж хвилі спіймав мене на сценарії без комміту red:. Він мав рацію; послаблювати його я не став, а назвав роботу тим, чим вона є. Доведено мутантом у коміті трансформи: поверни мертвий асерт — і проба назве файл, рядок і слово"
  functional.completeness: "тримає всі три сценарії; решта знахідок трьох аудитів названа поіменно в BACKLOG, а не змовчана — їх чотирнадцять, і кожна окрема робота"
...
**Застереження (§2.10): чому це chore, а не сценарій.** Клас, який він стереже, на момент написання
...
слушно не пустили комміт `red:` на зелений тест — а тоді **новий суд
цієї ж хвилі** спіймав мене на сценарії без червоного народження.
Обидва мали рацію.

Тож я не послабив суду, а назвав роботу тим, чим вона є: це
перевірка **над нашою батареєю**, не обіцянка про поведінку `keel`.
Червоне народження (§6.3) має сенс там, де є стан «падає до роботи»;
для суду проти класу, який зараз чистий, такого стану не існує.
Питання до норми — чи потрібен §6.3 виняток для судів над власними
пробами — винесено в BACKLOG.
