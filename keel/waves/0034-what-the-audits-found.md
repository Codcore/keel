...
**Застереження (§2.10): цей сценарій не має червоного народження, і
я цього не приховую.** Клас, який він стереже, на момент написання
**чистий** — єдиний мертвий асерт полагодила розсилка 0033. Тест,
який доводить, що суд працює, я прогнав **мутантом**: повернув той
самий асерт назад, і проба назвала його файлом, рядком і словом
(`verdict_limits_test.rs:214 — "межа вироку" (["вироку"])`). Ворота
слушно не пустили комміт `red:` на зелений тест, і я не став цього
обходити.

Це межа §6.3, а не моя недбалість: методика припускає, що в кожного
сценарію є стан, у якому він падає **до** роботи. Для суду, доданого
проти класу, який зараз не порушений, такого стану нема. Названо тут
і винесено в BACKLOG як питання до норми.
