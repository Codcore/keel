...
decisions:
  ...
  performance.time-behaviour: "свідомо без тесту, але ціна названа числом (R-12 рецензії): проба клонує репозиторій ДВІЧІ, тож батарея йде з 30,4 до 45,0 с (+48 %), а keel close — зі 119 до 146 с (+23 %), бо жене батарею тричі. Це ціна за суд над числом, якого доти не судив ніхто; дешевшого способу порівняти повний вирок з обрізаним я не знайшов"
  performance.capacity: "не застосовується"
  performance.resource-utilisation: "свідомо без тесту: червона проба лишає пісочницю 12 МіБ (два клони) замість кілобайтів, як у 0030 — це та сама школа «доказ лишається», але вдесятеро дорожча; названо тут, щоб не було несподіванкою на спільному диску"
...
|---|---|---|
| «межа вироку» (усі відмінки) | 8 | **не перевірено** — ось що |
| «перевірено цим **поверхом**» | 1 | перевірено **цією командою** |
| «**щабель** 1», «щабель 21», «щаблем» | 3 | сліди внутрішньої драбини bootstrap; людині вони не кажуть нічого |
| «межа зеленого» | 1 | лишаю: воно **пояснює себе в тому ж рядку** («зелений тест означає…») |

Слова «розріз» і «трансформа» лишаються — але з поправкою рецензента,
який перевірив обидва: «трансформа» **справді визначена** (§2.4,
дослівно: «порція роботи, яка наближає обіцянки хвилі»), а «розріз»
глава 10 вживає десять разів і **ніде не визначає**. Тож він
лишається не тому, що пояснений, а тому, що це слово методики, а не
моя вигадка на ходу; пояснити його одним реченням у `keel cuts` —
названий борг, не зроблений тут.
...
тому, скільки звірок насправді зробив повний. Не «більше нуля», не «якесь» — саме
те саме число. Зіграно шість мутантів: нуль, `+1`, `/2`, зняття
рядка, зняття межі свіжости бази — усі шість червонять. **Одного
проба не ловить, і це названо тут:** жорстка константа, що дорівнює
сьогоднішній правді (141), переживає її — доти, доки правда не
зміниться. Рецензент показав, що на гілці `0010-contracts-hold` те
саме порівняння дає 33, тобто одна константа не вгодить двом станам.
