...
adapter-ruby-unmarked = звіт minitest не стоїть між мітками keel: у ділянці, яку keel читає у "{ $file }", його підсумку нема. Ділянку закриває мітка, що друкується зсередини самого `Minitest.run`, одразу по звіту, — і три речі роблять цю відмову: мітка не дійшла зовсім; мітка стала раніше за підсумок (хтось покликав `Minitest.run` посеред бігу); або підсумок зник із потоку по дорозі
adapter-ruby-unmarked-instead = щось між minitest і keel переписує потік — власний репортер, плагін, `at_exit`, що підміняє STDOUT і не повертає його, або тіло тесту, яке само кличе біг. Прожени файл рукою і подивись, чи стоїть підсумок перед останнім рядком; читати такий потік далі keel не стане
...
adapter-ruby-report = звіт minitest не сходиться сам із собою: у "{ $file }" підсумок рахує { $counted } невдач і { $skips } скіпів, а названих блоків — { $blocks } і { $blockskips } відповідно. Вироки беруться з блоків, які minitest пише ПІСЛЯ бігу, числа — з його ж підсумку, і розбіжність означає, що один із двох написав не minitest
