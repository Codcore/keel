...
close-plan-no-wave = ця гілка зветься `plan/{ $wave }`, а хвилі з таким іменем у дереві нема (§8.2). План-гілка затверджується злиттям, і затверджувати тут нема чого: самого плану бракує. Те саме каже `keel check`, а код, покладений на таку гілку, однаково судиться за §4.9
close-lack-plan-review = план хвилі { $wave } не зустрів читача: keel/reviews/{ $wave }-plan.md нема в історії цієї гілки. §9.9 просить свіже око перед злиттям хвилі, а план затверджується злиттям власного PR (§6.6) — отже саме тут те око й читає. Ім'я несе `-plan` навмисно: звіт під власним іменем хвилі переїхав би на робочу гілку і задовольнив ворота, які існують заради рецензії РОБОТИ
close-lack-plan-review-empty = у плану хвилі { $wave } файл звіту є, і він порожній: keel/reviews/{ $wave }-plan.md нічого не несе, а порожній файл не рецензія (§9.9)
