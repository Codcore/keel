...
tags-spec-one-liner = тег proves: { $scenario }@{ $rev } стоїть над однорядковим прикладом `it { "{ … }" }` — rspec зве його місцем у файлі, і імені, яке б тримав тег, нема
...
limit-rspec-border = не перевірено: друге читання ruby — rspec — читає spec/**/*_spec.rb і зве приклад повним описом, як rspec (групи через пробіл, без пробілу перед #… і .…); один приклад біжить за ідентифікатором із `--dry-run`, вирок — із JSON, і pending — не бігло; однорядкові приклади `it { "{ … }" }` імені не мають і тега не тримають, приклади в shared_examples — теж; `describe` з не-літеральним аргументом (described_class) імені групи не дає; spec/support/ і spec_helper.rb не читаються; згенерований CI жене minitest — крок rspec цей реліз не пише, додай його сам
