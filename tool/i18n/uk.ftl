...
check-red-mutant = не перевірено: обіцянка "{ $scenario }" народилась зеленою за названим винятком §6.3 — у коміті записано, що зламано { $broke }, і проба назвала це так: { $named }. Машина не перевіряє, що мутант справжній; це слово автора, і його читає рецензент
check-wave-cancelled = не судиться: хвилю { $wave } скасовано — { $why } (§6)
check-scope-cancelled = scope не звірявся: гілка "{ $branch }" зветься як хвиля { $wave }, а її скасовано (§6)
close-cancelled = { $wave }: скасована — { $why } (§6): доводити нема чого, і суд її не чекає
status-wave-cancelled = { "  " }{ $wave } — скасована: { $why } (§6)
