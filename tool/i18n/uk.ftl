...
close-check-red = суд документів каже своє: { $count } знахідок `keel check` на власних файлах цієї гілки (§7.1–§7.9, глава 4) — вони блокери тут так само, як там, бо суд, що впускає в merge, не вужчий за той, що стоїть поруч
close-check-red-plan = суд документів каже своє: { $count } знахідок `keel check` на власних файлах цієї гілки (§7.1–§7.9, глава 4) — надруковані і НЕ полічені, бо хвиля цієї гілки затверджена і не почата: усе, що суд scope тут може сказати, — що робота ще не починалась, а план зливається як план (§6.6). Той самий список несе keel check зі своїм кодом виходу
