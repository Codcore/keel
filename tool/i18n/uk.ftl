...
limit-elixir-form = не перевірено по суті: elixir не пише типів у `def`, тож суд форми §7.6 звіряє імʼя і параметри — і тільки (§7.8). І межа, зміряна цією хвилею: `def` усередині `@moduledoc`/heredoc від живого не відрізняється — прибирач коментарів ріже `#` і трьох лапок не знає, тож обіцянку тримає рядок документації. Те саме в ruby (BACKLOG)
