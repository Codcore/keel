...
adapter-ruby-roll = перелік не сходиться з бігом: minitest каже, що прогнав { $said } у "{ $file }", а цей читач назвав { $read }. Рядок вироку несе посередині власний вивід тесту, і читач, який один загубить, губить його мовчки — тому число звіряється, а різниця кажеться вголос, а не вгадується
adapter-ruby-roll-instead = натомість: прожени файл із -v рукою і подивись на рядки довкола різниці; тест, що друкує під час бігу, цілком законний, і читач мусить це пережити — якщо не переживає, це вада keel і їй місце в issue
adapter-ruby-roll-lost = тест оголошено і не суджено: у "{ $file }" сам файл перелічує { $count } тест(ів), про які біг не сказав нічого — { $tests }. Перелік береться при ЗАВАНТАЖЕННІ файлу, ще до того, як побіжить бодай одне тіло, тож ніщо, надруковане тестом, до нього не додасться і з нього не зникне: ім'я без вироку означає, що вирок загубився — його з'їв вивід або його не було
adapter-ruby-roll-lost-instead = натомість: прожени файл із -v рукою і знайди тест, чий рядок розбито; це робить друк із тіла тесту рядка виду `Class#test_x = 0.00 s = .`
