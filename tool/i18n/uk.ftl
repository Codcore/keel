...
check-scope-base-first = перший commit гілки @ { $sha } (стовбура тут нема: ніхто його не назвав, і ні main, ні master не стоять)
check-trunk-named = стовбур: { $trunk } — його назвав keel.toml
check-trunk-refused = стовбур: { $trunk } — узято без відповіді git-а. git називає { $refused }, а та гілка несе власний файл хвилі — отже це гілка роботи, і стовбуром вона не буває (§8.2): так називає клон, знятий із робочого дерева. Якщо взятий вище стовбур — не та гілка, назви його в keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated])
check-trunk-refused-alone = стовбура не знайти. git називає { $refused }, а та гілка несе власний файл хвилі — отже це гілка роботи, і стовбуром вона не буває (§8.2): так називає клон, знятий із робочого дерева. Назви стовбур у keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated])
check-trunk-git = стовбур: { $trunk } — його назвав git: refs/remotes/{ $remote }/HEAD; якщо гілка не та: git remote set-head { $remote } -a, або назви стовбур у keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated])
...
limit-no-trunk = не перевірено: стовбура main цей клон не знає, тож точки розгалуження нема — зникнення документів (§4.12) і код на план-гілці (§4.9) не судились; назви його в keel.toml (trunk = "...") або принеси ту гілку з віддаленого
limit-base-is-head = не перевірено: стовбур, який називає цей клон, стоїть на цій гілці або попереду неї, тож база порівняння І Є вершина гілки — власного в неї порівняти нічого, і §4.4–§4.6, §4.9 та §4.12 не судились. Так називає стовбур клон, знятий із робочого дерева (`git clone` копіює HEAD джерела); назви стовбур у keel.toml (trunk = "...") або принеси ту гілку
...
close-awaiting-merge-unseen = { $wave }: легка — самі chore, закриється фактом merge (§6.5): стовбура тут не знайти, факту merge суд не бачить
...
status-wave-light-unseen = { "  " }{ $wave } — доводити нема чого: обіцянок у ній нема, тож закриється фактом merge (§6.5): стовбура тут не знайти, факту merge не видно
