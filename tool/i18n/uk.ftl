...
adapter-ruby-roll = перелік не сходиться з бігом: minitest каже, що прогнав { $said } тест(ів) у "{ $file }", а перелік імен, знятий при завантаженні файлу, налічує { $read }. Перелік друкується на той самий потік, куди файл пише, поки вантажиться, — тож ім'я до нього дописати можна, а змусити minitest його прогнати ні. Число minitest — його власне, і різниця кажеться вголос, а не вгадується
adapter-ruby-roll-instead = прожени файл рукою і подивись, які імена стоять у переліку понад ті, що побігли; рядок виду `KEEL-ROLL Class#test_x`, надрукований при завантаженні файлу, дає рівно цю різницю
adapter-rails-shape = вироку не зібрано: у "{ $file }" рядки `-v` не дають імені й мітки на кожен оголошений тест. Rails сам вантажить застосунок і веде біг, тож переліку імен, який keel бере на звичайній дорозі, тут нема, і читати лишається саму форму рядка. З цього файлу не взято жодного вироку
adapter-rails-shape-instead = прожени файл рукою і подивись на вивід. Форму рядка ламають три речі, і всі три зміряні: друк із тіла тесту (рядок вироку розривається надвоє); parallelize_me!, а Rails вмикає паралель сам (minitest фізично перемішує рядки двох тестів); і спековий стиль describe/it, де ім'я несе пробіли. Перше лікується тишею в тесті, друге — вимкненою паралеллю, третє поки що ні: переліку на цій дорозі ще нема, і рядок про це стоїть у BACKLOG
adapter-ruby-roll-none = переліку нема: minitest каже, що прогнав { $said } тест(ів) у "{ $file }", а рядків переліку, які keel друкує одразу після завантаження файлу, у виводі нема
adapter-ruby-roll-none-instead = файл під час завантаження підміняє STDOUT — звичайний спосіб приглушити гучний boot — і ковтає перелік разом із ним; поверни потік раніше або не глуши його зовсім. Читати вироки з форми рядка тут keel не стане: саме цю форму тест і підробляє
adapter-ruby-unsaid = перелік і бігун говорять про різні імена: перелік у "{ $file }" називає "{ $name }", а у всьому, що вийшло з бігуна, цього імені нема жодного разу. Під `-v` minitest пише ім'я кожного тесту, який побіг; вирок імені, про який бігун не сказав ні слова, — ніким не даний
adapter-ruby-unsaid-instead = прожени файл рукою з `-v` і подивись, чи з'являється ім'я; де його нема, біг не належить переліку, і зелене тут не доведене нікому
adapter-ruby-mute = ruby вийшов із помилкою і не сказав нічого, що keel міг би прочитати: у "{ $file }" нема ні підсумку minitest, ні жодного вироку, а код виходу не нульовий
adapter-ruby-mute-instead = так поводиться файл, який глушить STDOUT при завантаженні і не повертає його, і файл, що кидає виняток уже після оголошення тестів. Прожени його рукою: червоний вихід — це червоний вихід, і нуль тестів тут був би тихою неправдою
adapter-ruby-report = звіт minitest не сходиться сам із собою: у "{ $file }" жоден із підсумків у виводі бігуна не згоден з названими блоками — останній підсумок рахує { $counted } невдач і { $skips } скіпів, а названих блоків — { $blocks } і { $blockskips } відповідно. Вироки беруться з блоків, які minitest пише ПІСЛЯ бігу, числа — з його ж підсумку, і розбіжність означає, що один із двох написав не minitest
adapter-ruby-report-instead = прожени файл рукою і подивись на блоки «1) Failure:» під підсумком — друк такого блоку з тіла тесту дає рівно цю розбіжність
adapter-ruby-report-none = звіт для "{ $file }" не має жодного підсумку minitest: рядка виду «N runs, N assertions, …» у виводі бігуна нема, хоча minitest пише його після кожного бігу. Без підсумку числа блоків перевірити нічим — а блоки уміє писати й тест
adapter-ruby-report-none-instead = прожени файл рукою і знайди рядок підсумку; якщо його з'їдає заміна STDOUT або власний делегат у файлі — читати такий вивід keel не стане
