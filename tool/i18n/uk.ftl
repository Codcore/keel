...
adapter-ruby-roll-lost = тест названо і не суджено: у "{ $file }" цей читач відкрив { $count } імен, чий час так і не прийшов. Так буває, коли вивід тесту сам виглядає як рядок вироку — і тоді справжній тест міняється на той, якого не було, один до одного, а всі числа при цьому сходяться
