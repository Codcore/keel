...
check-trunk-gone = стовбур: { $trunk } — узято без відповіді git-а. git називає { $refused }, а тієї гілки вже нема: символьне посилання переживає гілку, на яку вказує, і імʼя, що ніде не стоїть, базою бути не може. Полагодь це `git remote set-head <віддалений> -a` або назви стовбур у keel.toml
check-trunk-gone-alone = стовбура не знайти. git називає { $refused }, а тієї гілки вже нема: символьне посилання переживає гілку, на яку вказує. Полагодь це `git remote set-head <віддалений> -a` або назви стовбур у keel.toml
...
close-awaiting-merge-unseen = { $wave }: легка — самі chore, закриється фактом merge (§6.5): факту merge суд не бачить — стовбура тут не знайти, або джерела кажуть про нього різне
...
status-wave-light-unseen = { "  " }{ $wave } — доводити нема чого: обіцянок у ній нема, тож закриється фактом merge (§6.5): факту merge не видно — стовбура тут не знайти, або джерела кажуть про нього різне
