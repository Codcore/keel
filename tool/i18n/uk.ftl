...
tags-not-utf8 = файл не читається як текст UTF-8: перші { $at } байтів прочиталися, далі — байт, якого в UTF-8 нема (файл, найпевніше, в latin-1 чи cp1251)
tags-not-utf8-instead = перекодуй файл у UTF-8 (`iconv -f latin1 -t utf8`) — суд читає сирці лише як UTF-8
...
rev-nearmiss = заголовок "## { $heading }" пише слово секції без пробілу — не розпізнаний як секція, але й не вільна проза (§7.7; суворіше за букву норми, бо вона судить секції, а не одруки в заголовках, — а такий заголовок інакше мовчав би)
...
rev-write-stopped = рука перепису спинилась: { $count } { $count ->
    ...
    [few] знахідки
   *[many] знахідок
} вище — нічого не записано далі, і код виходу каже те саме
rev-write-only-kept = переписувати не було чого: { $count } { $count ->
    [one] запис розійшовся
    [few] записи розійшлись
   *[many] записів розійшлись
}, і всі вони — у закритих хвилях, де стара редакція законна (§5.6)
rev-write-none = у відкритих хвилях нічого не розійшлось — кожен їхній запис чинний
rev-write-count = записів переписано: { $count }

## модуль graph
graph-scenario-twice = сценарій "{ $scenario }" живе більш ніж в одній хвилі: { $waves } — тег тесту голий, тож машина не знає, чию обіцянку він доводить, і один тест закриває обидві (суворіше за букву норми, бо вона не забороняє двом хвилям одне імʼя, — а суд не вгадує, чию обіцянку доведено)
graph-scenario-twice-instead = дай сценаріям різні імена: одне імʼя — один дім
graph-name-taken = імʼя "{ $name }" носить і обіцянка хвилі { $wave }, і контракт — а тег тесту голий, тож не видно, чию редакцію він тримає (суворіше за букву норми, бо вона не ділить імен між обіцянками й контрактами, — а суд не вгадує, чию редакцію тримає тег)
...
graph-double-cover = розріз "{ $slug }" має { $count } живих covers: сценарії { $holders } (§10.3 — рівно одна відповідь; суворіше за букву норми, бо вона просить відповіді на кожен розріз, а не забороняє двох, — а дві живі covers ховають, котра обіцянка судиться)
graph-double-cover-instead = лиши один cover; другий сценарій стоїть на своєму proves або іншому розрізі (§3.3)
graph-crossing = рядок scope "{ $file }" оголошують дві відкриті хвилі — { $first } і { $second } — а ребра depends_on між ними нема: їхні scope перетинаються, а перетин між незалежними хвилями — питання ще на плануванні (§8.8)
graph-crossing-instead = назви залежність depends_on в одній із хвиль (тоді вони йдуть одна за одною) або поділи файл між ними ще на плануванні (§8.8)
...
graph-no-reason = відповідь без причини: { $cuts } — сама формула і нічого більше; §10.3 просить причину: «не застосовується, БО…» або «свідомо не робимо, БО…»
graph-no-reason-instead = допиши причину до кожного з названих розрізів — одне речення про те, ЧОМУ цей розріз тут не грає; закритих хвиль це не стосується, історію не переписують
...
scope-outside = оголошений файл "{ $file }" лежить поза деревом проєкту — scope називає файли цього дерева (§4.1)
scope-outside-instead = напиши шлях від кореня проєкту без "..", або, якщо файл справді чужий, він не файл цієї хвилі
...
scope-light-contract = гілка легкої хвилі { $wave } змінює контракт "{ $contract }" — контракт створює чи змінює лише повна хвиля (§6.8, §5.7), і другого людського погляду тут не було
scope-light-contract-instead = назви контракт у files трансформи — тоді вага повна і хвиля їде двома PR (§8.1) — або прибери зміну контракту з гілки
scope-light-contract-instead-chores = хвиля без жодної обіцянки контракту не змінює (§2.11): дай їй сценарій — або прибери зміну контракту з гілки
scope-transform-uncommitted = трансформа "{ $name }" торкнута в усіх названих файлах, а коміту з темою `{ $name }: …` на гілці нема — трансформу закриває коміт під її слагом (§6.2)
scope-transform-uncommitted-instead = закоммить роботу трансформи під її слагом: `{ $name }: <слова>` — кілька комітів дозволено (§2.4); на main цього суду нема — там історія судиться наслідками (§6.5)
...
tags-js-dynamic = тег proves: { $scenario }@{ $rev } стоїть над тестом, чиє імʼя node будує під час бігу (шаблон із { "${…}" }) — з сирця його не прочитати
tags-js-dynamic-instead = назви тест звичайним рядком (§5.5): тег тримає імʼя, а імʼя мусить бути відоме до бігу
tags-js-subtest = тег proves: { $scenario }@{ $rev } стоїть над підтестом (t.test) — node жене його лише крізь батьківський test, і сам по собі він не біжить
tags-js-subtest-instead = постав тег на батьківський test(…) (§5.5): його вирок несе всі підтести — або зроби підтест окремим test
tags-spec-one-liner = тег proves: { $scenario }@{ $rev } стоїть над прикладом без імені — `it { "{ … }" }` чи `it do … end` — rspec зве його місцем у файлі, і імені, яке б тримав тег, нема
tags-spec-one-liner-instead = дай прикладу імʼя — `it "…" do … end` (§5.5): тег тримає повний опис, а його складає rspec з імен
...
adapter-rspec-silent = rspec вийшов, не написавши JSON — ось що він сказав: { $error }
...
holding-vanished = контракт "{ $contract }" обіцяє "{ $name }" — такої одиниці у { $file } нема (§7.6); те, що живе лише в коментарі чи в багаторядковому тексті, не рахується сирцем
...
review-drift-header-fork = ## Дрейф (§4.6) — файли, дописані в scope після якоря (файл хвилі в точці розгалуження з main, після merge план-PR: { $sha })
...
review-plan-title = keel review — пакет ПЛАНУ (§9.9 до злиття §6.6)
review-plan-why = сорок відповідей пишуться тут, і свіже око читає їх ЗАРАЗ — доки план ще можна відхилити як план (§6.6). Машина ловить ВІДСУТНІСТЬ відповіді; неправду в ній ловить лише читач, і його звіт лягає в keel/reviews/{ $wave }-plan.md
review-plan-covered-header = розрізи, закриті обіцянкою — чи ця обіцянка справді доводить ЦЕЙ розріз?
review-plan-covered = "{ $scenario }" закриває: { $cuts }
review-plan-covered-body = { "    " }обіцяє: { $body }
review-plan-body-empty = (секції цієї обіцянки в тілі хвилі нема — читати нема чого, і це вже знахідка)
review-plan-more = { "  " }…і ще обіцянок: { $count } — решту читай у самому файлі хвилі
review-plan-silent = розрізи БЕЗ відповіді: { $cuts } — це вже червонить keel check (§10.3); поки їх нема, читати решту рано
review-plan-unknown = гілка "{ $branch }" зветься планом, але слаг після plan/ не називає жодної прочитаної хвилі
review-plan-unknown-instead = звір імʼя гілки з файлом у keel/waves/ — план і робота звуться однаково, різняться лише префіксом plan/ (§8.2)
review-plan-decided-header = розрізи, вирішені без причини — це причина чи відмашка?
review-plan-decided = { $cut }: "{ $said }"
review-plan-decided-more = { "  " }…і ще { $count }: §10.3 просить причину — «не застосовується, БО…», — а не саму формулу; якщо їх стільки, питання не до окремого розрізу, а до того, як писався цей план
review-plan-crowded = обіцянок { $scenarios }, а розрізів вони закривають { $cuts }: одне рішення трьома голосами — привід перечитати, чи не притягнуто пару там, де слот був вільний
review-plan-nothing = питань нема: жодна обіцянка не закриває розрізу, жодна відповідь не є голою формулою, і всі сорок відповіджені. Це звичайна форма хвилі-chore (реліз, журнал); читати лишається саму картку
review-plan-footer = кожне питання вище — «так чи ні, і чому». Відповідає людина або свіжий агент; інструмент лише каже, де дивитись. Звіт цього читання — keel/reviews/{ $wave }-plan.md, і без нього план-PR не зливається (§9.9)
...
review-transform-no-body = (секції тіла нема — суд §7.7 «шапка↔тіло» на крок попереду; суди руками)
...
adapter-rspec-printed = — і те, що сам процес написав, доки біг цей файл, дослівно:
adapter-rspec-tmp = тимчасову теку для JSON rspec не створити: { $error }
adapter-rspec-tmp-instead = тека мусить бути новою і своєю: або щось уже стоїть під цим іменем у тимчасовій теці системи — прибери його, — або самої тимчасової теки (TMPDIR) нема чи в неї не пишеться — створи її або зміни TMPDIR; тоді повтори
...
adapter-elixir-name-not-text = бігун elixir надрукував імʼя тесту "{ $name }" не текстом: воно несе екранні послідовності або знак заміни, тож не збігається з жодним іменем у файлі — а суд, прочитавши його, сказав би, що батарея пропустила тест, який вона виконала
adapter-elixir-name-not-text-instead = бігунові сказано `-kernel standard_io_encoding unicode`, але власне ELIXIR_ERL_OPTIONS цього проєкту стоїть першим і виграло — прапор бʼє й локаль. Прибери звідти слово про кодування, і імена дійдуть
adapter-elixir-failed-instead = постав elixir і mix на PATH — адаптер кличе mix командою системи, лише додає йому `-kernel standard_io_encoding unicode`, щоб імена тестів доходили текстом
adapter-elixir-broken = проєкт не компілюється: { $error }
adapter-elixir-broken-instead = mix каже це кодом виходу 1, а падіння — кодом 2; без збірки вироку нема нікому — полагодь і повтори
...
gate-red-pass-ruby = народження червоного "{ $scenario }": тест "{ $test }" падає — а падіння від незібраного ruby кодом виходу не розрізняє: суд читав рядки minitest чи JSON rspec, а де їх не досить, падіння приймається падінням (§7.12) — commit проходить
gate-red-pass = народження червоного "{ $scenario }": тест "{ $test }" справді падає — commit проходить (§7.12)
gate-red-mutant = народження зеленого "{ $scenario }": тест "{ $test }" зелений, і це названий виняток §6.3 — у коміті записано мутанта: зламано { $broke } → проба назвала це так: { $named }. Машина не перевіряє, що мутант справжній: це слово автора, і його читає рецензент
gate-red-green = заявлене народження червоного "{ $scenario }", але тест "{ $test }" зелений — незароблене «бачив червоним» в історію не вʼїжджає (§7.12). Якщо це суд над власною батареєю чи інструментарієм, який не можна побачити червоним, не зламавши того, що він стереже, — це названий виняток §6.3: додай у повідомлення рядок `mutant: <що зламано> -> <як проба це назвала>`
gate-red-green-instead = зроби тест таким, що справді падає; а якщо це суд над власною батареєю чи інструментарієм — додай у повідомлення рядок `mutant: <що зламано> -> <як проба це назвала>` (§6.3)
gate-red-unknown = red: називає "{ $slug }" — це не сценарій хвилі { $wave }
gate-red-unknown-instead = назви сценарій цієї хвилі (котрий народжується — покаже keel next) або впиши його у файл хвилі
gate-red-withdrawn = "{ $scenario }" знятий — мертва обіцянка не народжується (§2.12)
gate-red-withdrawn-instead = народжуй живий сценарій, або зніми withdrawn у файлі хвилі, якщо обіцянка знову жива
gate-red-untagged = заявлене народження червоного "{ $scenario }", але жоден тест не несе його тега proves (§5.5)
gate-red-untagged-instead = постав над тестом рядок `proves: <сценарій>@<редакція>` — редакцію дає keel rev
gate-red-many-tags = "{ $scenario }" несе { $count } тегів proves — котрий народжується, не вгадується
gate-red-many-tags-instead = лиши один тег proves цього сценарію, а решті дай їхні власні сценарії
gate-red-broken = заявлене народження червоного "{ $scenario }", але тести не збираються — злам збірки не є падінням (А3): { $words }
gate-red-broken-instead = полагодь збірку тестів і повтори commit: падіння судиться лише там, де є чому бігти
...
gate-red-notrun-instead = звір імʼя тесту з тегом і перевір, що біг справді його виконує — рядок запуску дає keel next
...
gate-work-red-instead = доведи роботу до зеленого тесту, або закоммить спершу народження `red: <сценарій>`
gate-work-stale = трансформа "{ $transform }": тег сценарію "{ $scenario }" тримає { $recorded }, а текст дає { $actual } (§7.5)
gate-work-stale-instead = пересунь теги командою `keel rev --write`, якщо текст сценарію змінено свідомо, або поверни текст сценарію
gate-work-untagged = трансформа "{ $transform }": сценарій "{ $scenario }" не має тега proves у тестах (§5.5)
gate-work-untagged-instead = постав над тестом рядок `proves: <сценарій>@<редакція>` (редакцію дає keel rev) і закоммить народження `red: <сценарій>`
gate-work-broken = трансформа "{ $transform }": тести не збираються: { $words }
gate-work-broken-instead = полагодь збірку тестів і повтори commit
gate-work-notrun = трансформа "{ $transform }": біг не виконав жодного тесту "{ $test }" сценарію "{ $scenario }"
gate-work-notrun-instead = звір імʼя тесту з тегом і перевір, що біг справді його виконує — рядок запуску дає keel next
gate-unknown-slug = "{ $slug }" — не red: і не трансформа хвилі { $wave }; одрук не проходить як «поза судом» (§8.4)
gate-unknown-slug-instead = назви слаг трансформи цієї хвилі (котрої — покаже keel next) або напиши тему без слага: тоді commit іде поза судом (§8.4)
gate-case = "{ $head }" носить великі літери — red: і слаги пишуться малими (§1.2, §8.4); капіталізований двійник не проходить як «поза судом» (суворіше за букву норми, бо вона називає форму, а не забороняє великих літер, — а двійник частіше одрук, ніж чужа тема)
gate-case-instead = напиши початок теми малими літерами — слаги пишуться малими (§1.2)
...
gate-adapter-refuses = адаптер "{ $name }" не цього релізу (реліз веде { $known }) — народження чи роботу трансформи цей суд судити не може, а суд, який не може судити, не пускає (§7.12)
gate-adapter-refuses-instead = постав реліз, що веде цей адаптер, або назви в keel.toml адаптер із переліку
...
## команда check
check-title = keel check — документи
...
check-red-unjudged = не перевірено: червоних народжень на цій гілці не звірити — історія до бази порівняння не досяжна (§6.3, §7.12)
check-red-unjudged-instead = щоб перевірити: git fetch --unshallow (у CI — fetch-depth: 0)
check-red-unborn = сценарій "{ $scenario }" має тест на цій гілці, але комміту "red: { $scenario }" на ній нема — зелений тест, якого не бачили червоним, не доводить нічого (§6.3, §7.12)
check-red-unborn-instead = народи його червоним: доведи тест до падіння, закоміть "red: { $scenario }", і лише тоді роби роботу
...
check-scope-base-main = merge-base зі стовбуром { $trunk } @ { $sha }
check-scope-base-first = перший commit гілки @ { $sha } (стовбура тут нема: ніщо з того, що цей клон міг би назвати, не стоїть)
check-trunk-named = стовбур: { $trunk } — його назвав keel.toml
check-trunk-refused = стовбур: { $trunk } — узято без відповіді git-а. git називає { $refused }, а це гілка РОБОТИ — план, spike або гілка, що несе власний файл хвилі (§8.2, §4.13), — і стовбуром гілка роботи не буває. Такої форми відповідь дає клон, знятий із робочого дерева. Якщо взятий вище стовбур — не та гілка, назви його в keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated])
check-trunk-refused-alone = стовбура не знайти. git називає { $refused }, а це гілка РОБОТИ — план, spike або гілка, що несе власний файл хвилі (§8.2, §4.13), — і стовбуром гілка роботи не буває. Такої форми відповідь дає клон, знятий із робочого дерева. Назви стовбур у keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated])
check-trunk-gone = стовбур: { $trunk } — узято без відповіді git-а. git називає { $refused }, а тієї гілки вже нема: символьне посилання переживає гілку, на яку вказує, і імʼя, що ніде не стоїть, базою бути не може. Полагодь це `git remote set-head { $remote } -a` або назви стовбур у keel.toml
check-trunk-gone-alone = стовбура не знайти. git називає { $refused }, а тієї гілки вже нема: символьне посилання переживає гілку, на яку вказує. Полагодь це `git remote set-head { $remote } -a` або назви стовбур у keel.toml
check-trunk-git = стовбур: { $trunk } — його назвав git: refs/remotes/{ $remote }/HEAD; якщо гілка не та, назви стовбур у keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated]) — а там, де віддалений це сервер, а не робоче дерево, те саме навчить цей клон git remote set-head { $remote } -a
check-trunk-guess = стовбур: { $trunk } — його не назвав ніхто, тож узято за іменем (main, потім master). Якщо гілка не та: git remote set-head { $remote } -a; а там, де тека народжується наново щопрогону (CI), це не порятунок — назви стовбур у keel.toml: trunk = "..." — це КОРЕНЕВИЙ ключ, і стоїть він ДО [trust] та [generated], інакше читається як їхня частина
check-trunk-guess-alone = стовбур: { $trunk } — його не назвав ніхто, а віддаленого, щоб спитати, цей клон не має, тож узято за іменем (main, потім master). Якщо гілка не та, назви стовбур у keel.toml: trunk = "..." — це КОРЕНЕВИЙ ключ, і стоїть він ДО [trust] та [generated], інакше читається як їхня частина
limit-shallow-diff = не перевірено: історія обрізана, тож зникнення документів (§4.12) і код на план-гілці (§4.9) ні з чим порівнювати
limit-no-base = не перевірено: точки розгалуження цей клон не дає — зникнення документів (§4.12) і код на план-гілці (§4.9) не судились
limit-no-trunk = не перевірено: стовбура цей клон не знає, тож точки розгалуження нема — зникнення документів (§4.12) і код на план-гілці (§4.9) не судились; назви стовбур у keel.toml (trunk = "...") або принеси ту гілку з віддаленого
check-red-mutant = не перевірено: коміт народження обіцянки "{ $scenario }" несе рядок мутанта за §6.3 — зламано { $broke }, і проба назвала це так: { $named }. Чи взяв цей коміт виняток, чи тест і так падав, знав лише hook у ту мить; машина не перевіряє ні того, ні що мутант справжній — це слово автора, і його читає рецензент
check-wave-cancelled = не перевірено: хвилю { $wave } скасовано — { $why } (§6.3-а) — не судиться
check-scope-cancelled = scope не звірявся: гілка "{ $branch }" зветься як хвиля { $wave }, а її скасовано (§6.3-а)
close-cancelled = { $wave }: скасована — { $why } (§6.3-а): доводити нема чого, і суд її не чекає
status-wave-cancelled = { "  " }{ $wave } — скасована: { $why } (§6.3-а)
docs-cancelled-empty = поле cancelled порожнє: скасування без причини — не скасування (§6)
docs-cancelled-empty-instead = напиши, чому хвилю не роблять: цей рядок читає людина, і він лишається в репозиторії назавжди
gate-cancelled = хвилю { $wave } скасовано — { $why } (§6.3-а): цей суд її не судить, і commit проходить
next-cancelled = хвилю { $wave } скасовано — { $why } (§6.3-а): роботи тут нема. Візьми іншу хвилю або сплануй нову
check-untested = обіцянка "{ $scenario }" не має жодного тегу тесту, а на гілці вже є робочі коміти (§7.5)
check-untested-instead = народи тест червоним (`red: { $scenario }`) і додай йому тег proves — або познач обіцянку withdrawn, якщо вона більше не потрібна (§2.12)
check-scope-spike = гілка "{ $branch }" — дослідження (§4.13): документів на ній не судять, і в main вона не зливається ніколи
close-spike = гілка "{ $branch }" — дослідження, а дослідження не зливається (§4.13)
close-spike-instead = поверни знахідку хвилею: розділ Why тієї хвилі — і є слід дослідження
word-weight-light = легка
word-weight-full = повна
status-weight = вага { $weight } (§6.8) — виведено з файлу, не проставлено рукою
scope-full-one-branch = хвиля { $wave } — { $weight } (§6.8), а її файл народився на цій самій гілці разом із роботою: тоді двох людських поглядів, яких вимагає §6.8, не було
scope-full-one-branch-instead = повна хвиля планується окремою гілкою plan/{ $wave } і окремим PR (§8.1); якщо вага вийшла повною ненавмисно — прибери контракт або withdrawn чи розбий на трансформи
scope-vanished = документ "{ $slug }" зник із гілки, а обіцянки не вмирають зникненням файлу (§4.12, §2.12)
scope-vanished-instead = або поверни файл і познач обіцянки withdrawn, або дай новому документові рядок renamed_from: { $slug }
scope-two-heirs = зниклий документ "{ $slug }" заявляють своєю спадщиною одразу двоє: { $heirs } — старе імʼя не веде до обох (§4.12)
scope-two-heirs-instead = лиши renamed_from лише в одного з них
scope-moved-across = документ "{ $slug }" переїхав в іншу теку: спадщину заявляє "{ $heir }" (§4.12)
scope-moved-across-instead = хвиля не стає контрактом і навпаки — зроби новий документ без renamed_from, а старий познач withdrawn
scope-plan-code = { $file } — план-гілка несе план, а не код (§4.9): цього файлу тут бути не може
scope-plan-code-instead = перенеси зміну на робочу гілку хвилі; на план-гілці лишаються тільки документи методики (§4.8)
check-scope-plan-unjudged = план-гілку "{ $branch }" (хвиля { $wave }) за §4.9 не судили: порівнювати нема з чим — причина в переліку «не перевірено» вище
check-scope-plan = план-гілка "{ $branch }" планує хвилю { $wave } — судиться за §4.9: тільки документи методики
check-scope-plan-nowave = гілка "{ $branch }" зветься план-гілкою хвилі { $wave }, а такої хвилі нема (§8.2) — код на ній однаково судиться за §4.9
...
check-number-twice = число { $number } тримають кілька хвиль на диску ({ $count }): { $files } — одне число, одна хвиля (§8.8)
check-generated-stale = записаний відбиток згенерованого файлу "{ $file }" застарів: записано { $recorded }, файл дає { $actual }
check-generated-stale-release = текст — той, що пише цей реліз: `keel update` перепише запис; поки запис чужий, файл не впізнати як меблі (§4.8)
check-generated-stale-hand = файл правлений рукою — це код, не меблі (§4.8): поверни його `keel update`-ом, як каже його слово, або запиши нове свідомо
check-chores-heavy = хвиля { $wave } — самі chore, а вага повна ({ $why }): хвиля з самих chore мусить бути легкою (§2.11)
check-chores-heavy-transforms = трансформ { $count }, легка має одну
check-chores-heavy-contract = створює чи змінює контракт у files
check-chores-heavy-instead = велика робота без жодної обіцянки — привід зупинитись: лиши одну chore-трансформу або дай хвилі сценарій (§2.11)
check-no-documents = документів ще нема
check-checked-head = що саме перевірено:
check-checked-headers = шапки — словник і форма (глави 2–4, §7.9)
check-checked-refs = посилання на контракти і їхні редакції (§7.1, §7.3), стара редакція судиться по історії файлу для закритих хвиль (§5.6)
check-checked-graph = звʼязки графа (глава 3: розрізи, тиша, implements, depends_on, наступники, перетини scope відкритих хвиль; §7.2, §8.8, §10.3)
check-checked-scope = scope гілки, що зветься як хвиля (§4.1, §4.4–§4.6, §4.8; поза порівнянням, як меблі: keel/, keel.toml, згенеровані файли у формі цього релізу, lock-файл мови і те, що лишає її бігун — теки на кшталт __pycache__/ чи теки збірки)
check-checked-tags = редакції сценаріїв у тегах тестів (§5.5, §7.5) і зниклі теги проти точки розгалуження (§7.15)
check-checked-trust = довіра команд verify/ci проти записаних відбитків (§7.16, §2.8)
check-checked-holding = тримання форми контрактів (§7.6, §2.9)
check-checked-body = шапка↔тіло в обидва боки (§7.7)
check-checked-close = закриття судить keel close (§6.5)
check-unjudged = не перевірено: { $what }
...
# fewer checks.
limit-shallow = не перевірено: історія обрізана (shallow-клон) — { $skipped ->
        [one] { $skipped } стара редакція не звірена
        [few] { $skipped } старі редакції не звірені
       *[many] { $skipped } старих редакцій не звірено
    } (редакцій, не посилань — як лічить і повний клон), і скільки з них МОГЛА б звірити ця глибина — не рахується; натомість: git fetch --unshallow
...
limit-trunk-named-missing = не перевірено: keel.toml називає стовбуром "{ $trunk }", а гілки з таким іменем цей клон не має — ні власної, ні віддаленої; зникнення документів (§4.12) і код на план-гілці (§4.9) не судились. Виправ імʼя або принеси ту гілку
limit-hook-absent = не перевірено машиною: keel.toml каже hooks = true, але в цьому клоні нашого commit-msg hook-а нема — git hook-ів не клонує, тож блок в AGENTS.md обіцяє машину, якої тут нема: тут обидва правила (§8.4, §7.12) тримають люди; натомість: keel hook
limit-tags-cancelled = не перевірено: тест "{ $test }" несе тег сценарію "{ $scenario }" хвилі { $wave }, яку скасовано — скасована хвиля поза судом ціла (§6.3-а), тож це не сирота; натомість: прибери тест разом із гілкою хвилі або поверни хвилю в роботу
limit-ruby-border = не перевірено: ruby не відрізняє «впав» від «не зібрався» кодом виходу — обидва 1 (§7.12). Адаптер читає текст (SyntaxError, LoadError); де тексту не досить, падіння приймається падінням — напрям, який не робить із червоного зеленого; пропущений тест (`skip`, позначка S) — не біг: ні зелений, ні червоний
...
limit-elixir-border = зміряно: ця мова відрізняє «впав» від «не зібрався» кодом виходу (0 зелене, 2 падіння, 1 не скомпілювалось), тож межа §7.12 про нерозрізненність тут не стоїть — суд бере злам збірки зламом, а не червоним тестом; пропущений тест (`@tag :skip`) виходить із 0 і — не біг: ні зелений, ні червоний
limit-elixir-form = не перевірено по суті: elixir не пише типів у `def`, тож суд форми §7.6 звіряє імʼя і параметри — і тільки; зелена форма тут ще менше означає сенс, ніж у мові з типами (§7.8)
limit-elixir-unread = не перевірено: { $file } адаптер не читає — ExUnit зве тести *_test.exs, і тег звідти не прочитано
limit-ruby-unread = не перевірено: у test/ і spec/ лежить { $count } .rb-файлів, які адаптер не читає — він читає *_test.rb (minitest) і *_spec.rb (rspec): { $files }
...
check-summary = підсумок: { $docs ->
       ...
       *[many] { $refusals } знахідок
        ...
        [0] { "" }
        [one] , { $limits } річ не перевірено (вище)
        [few] , { $limits } речі не перевірено (вище)
       *[many] , { $limits } речей не перевірено (вище)
    }
...
check-next-rung = наступний крок: усе зелене — план наступної хвилі (§6.6) або робота за поточною: keel next
...
close-said-of-file = — що сказав бігун, доки біг "{ $file }", дослівно і без маскування:
close-said-of-files = — що сказали бігуни, доки бігли ці { $count } { $count ->
    [one] файл
    [few] файли
   *[many] файлів
 }, дослівно і без маскування, і текст у них один і той самий: { $files }
close-said-outside = — і те, що прогін сказав поза будь-якою ціллю (біг { $run } із { $runs }): cargo жене всі цілі одним процесом, і в його stderr вони пишуть по черзі, тож цей текст не належить жодному файлу. Власний банер cargo звідси знято — `Compiling`, `Running <бінарник>`, перелік упалих цілей: це його бухгалтерія, а не слова падіння. Лишається те, що написав хтось інший, найчастіше процес-дитина тесту:
close-test-said-run = (біг { $run } із { $runs })
close-test-said-nothing = — і бігун не сказав нічого, що цей суд міг би зберегти: під цим червоним його голос порожній. Так буває на дорозі rspec, де голос будується з полів JSON, а не тримається сирим: приклад, що впав із анонімним класом винятку і порожнім повідомленням, не лишає в документі жодного слова. Прожени файл рукою — процес міг сказати більше, ніж потрапило в документ
close-battery = батарея: { $count } тестів × { $runs } біги (§7.13) — зелений лише зелений у кожному бігу
close-check-red = суд документів каже своє: знахідок `keel check` на власних файлах цієї гілки (§7.1–§7.9, глава 4) — { $count }: вони блокери тут так само, як там, бо суд, що впускає в merge, не вужчий за той, що стоїть поруч
close-check-red-plan = суд документів каже своє: знахідок `keel check` на власних файлах цієї гілки (§7.1–§7.9, глава 4) — { $count }, з них полічено блокерами — { $counted }. Хвиля цієї гілки затверджена і не почата, і знахідки, які кажуть єдине — що оголошена робота ще не починалась (§4.4, §4.1), — друкуються і не лічаться: це той самий стан, який оголошує звіт нижче, а план зливається як план (§6.6). Усе інше лічиться тут так само, як деінде: дрейф, обіцянка, над якою працювали без тега, трансформа, якої не закриває коміт, контракт, вирощений легкою хвилею
close-check-blockers = знахідок суду документів на власних файлах цієї гілки — { $count }: вихід червоний
close-check-red-instead = натомість: полагодь названі вище файли і повтори keel close; те саме по всьому дереву окремо каже keel check
close-check-red-plan-instead = натомість: із ЦИМИ робити нічого — вони кажуть, що робота ще не починалась, а план зливається як план (§6.6); вони повернуться блокерами на робочій гілці, де на них і відповідають. Усе інше, що лічить цей звіт, стоїть саме по собі
close-closed = { $wave }: закрита — кожен живий сценарій доведений, посилання сходяться, звіт рецензії лежить в історії гілки (машина його не читала: що в ньому написано — слово рецензента, і його читає людина)
close-closed-unjudged = { $wave }: закрита — кожен живий сценарій доведений, звіт рецензії в історії; посилань не звірено: { $count } — історія тут не свідчить (§5.6)
close-closed-light = { $wave }: закрита (легка) — самі chore, закрита фактом merge (§6.5): файл хвилі стоїть у стовбурі
close-awaiting-merge = { $wave }: легка — самі chore, закриється фактом merge (§6.5): файла хвилі в стовбурі ще нема, merge і є закриття
close-awaiting-merge-unseen = { $wave }: легка — самі chore, закриється фактом merge (§6.5): факту merge суд не бачить — стовбура тут не знайти, або джерела кажуть про нього різне; назви стовбур у keel.toml (trunk = "..."), і факт стане видно
close-held-by-red = { $wave }: НЕ закривається — батарея цього дерева червона ({ $count }), а закриття означає «проходить» (§7.8); імена вище
...
close-plan-no-wave = ця гілка зветься `plan/{ $wave }`, а хвилі з таким іменем у дереві нема (§8.2). План-гілка затверджується злиттям, і затверджувати тут нема чого: самого плану бракує. Код, покладений на таку гілку, однаково судиться за §4.9, і `keel review` відмовляє тут із тією ж причиною
close-lack-plan-review = план хвилі { $wave } не зустрів читача: keel/reviews/{ $wave }-plan.md нема в історії цієї гілки. §9.9 просить свіже око перед злиттям хвилі, а план затверджується злиттям власного PR (§6.6) — отже саме тут те око й читає. Ім'я несе `-plan` навмисно: звіт під власним іменем хвилі переїхав би на робочу гілку і задовольнив ворота, які існують заради рецензії РОБОТИ; натомість: `keel review` на цій гілці збирає пакет ПЛАНУ для свіжого агента — поклади звіт у той файл і закоммить
close-lack-plan-review-empty = у плану хвилі { $wave } файл звіту є, і він порожній: keel/reviews/{ $wave }-plan.md нічого не несе, а порожній файл не рецензія (§9.9); натомість: поклади в нього звіт свіжого агента і закоммить
close-lack-review-empty = файл звіту рецензії keel/reviews/<хвиля>.md в історії є, але він порожній — порожній файл не рецензія (§9.9); натомість: поклади в нього звіт свіжого агента і закоммить
close-lack-review = звіту рецензії keel/reviews/<хвиля>.md нема в історії гілки (§9.9 кладе запис у неї, і файл, якого не несе жоден комміт, зникає з merge); натомість: закоммить його темою chore цієї хвилі або темою без слага
...
close-price = ціна цього суду: батарея жене тричі (§7.13) у ВЛАСНИЙ { $target } — успадкований кеш зсуває вироки (§6.7), тож це рішення, а не вада; місця треба ~{ $needed } ГіБ (зміряно на дереві самого keel; на цьому дереві ціна своя — скільки вийшло, каже рядок «ціна сплачена» нижче)
...
close-no-room = на диску вільно { $free } ГБ, а цьому судові треба ~{ $needed } ГБ — краще відмовитись зараз, ніж померти на півдорозі з «no space left on device» (суворіше за букву норми, бо вона не міряє диска, — а суд, що помер на півдорозі, не виносить вироку)
...
close-blockers-light = блокери хвилі цієї гілки { $wave }: { $count } — легка хвиля не зливається з нестачами, названими вище (§6.5, §9.9)
...
close-verify-failed = verify "{ $command }" контракту { $contract } — ВПАЛА — зламана чужа обіцянка не зливається (§2.8)
{ $words }
close-verify-untrusted = verify "{ $command }" контракту { $contract } — не бігала: недовірена (§7.16), тож обіцянку контракту ніхто не довів; той вирок тримає check; натомість: перечитай команду і запиши довіру рукою keel trust
close-red-blockers = батарея бачила червоне: { $count } — доки суд дивився, вони падали, тож хвиля не закривається; це не про те, чи заявляє їх сценарій: суд, який бачив червоне і закрив, гірший за суд, який не біг
close-verify-blockers = зламаних чужих обіцянок: { $count } — вихід червоний
close-verify-unproven = обіцянок контрактів, чий доказ не біг: { $count } — недовірена команда не біжить (§7.16); вирок недовіри тримає check, і поки він червоний, дерево не зливається — тож «блокерів нема» тут не кажуть; натомість: keel trust
close-ci-unproven = власні ворота проєкту (ci) не бігли: команда недовірена (§7.16) — це дерево ними не суджено; вирок недовіри тримає check, і поки він червоний, дерево не зливається; натомість: keel trust
close-verify-no-words = команда не лишила слів
close-ci-passed = ci "{ $command }" — пройшов: власний gate проєкту зелений
close-ci-failed = ci "{ $command }" — ВПАВ — власний gate проєкту червоний, хвиля не зливається (§7.16)
{ $words }
close-ci-untrusted = ci "{ $command }" — не бігав: недовірений (§7.16), тож власні ворота проєкту цього дерева не судили; той вирок тримає check; натомість: перечитай команду і запиши довіру рукою keel trust
...
map-project-cancelled = хвилю { $wave } скасовано — { $why } (§6.3-а): її відповіді не рахуються
...
status-wave-closed = { "  " }{ $wave } — закрита за структурою: теги збігаються, посилання сходяться, звіт рецензії лежить в історії гілки
status-wave-closed-unjudged = { "  " }{ $wave } — закрита за структурою; { $count } посилань не судили: історія тут не свідчить (§5.6)
status-wave-closed-light = { "  " }{ $wave } — доводити нема чого: обіцянок у ній нема, тож закрита фактом merge (§6.5): файл хвилі стоїть у стовбурі
status-wave-light-own = { "  " }{ $wave } — доводити нема чого: обіцянок у ній нема, тож закриється фактом merge (§6.5): файла хвилі в стовбурі ще нема
status-wave-light-unseen = { "  " }{ $wave } — доводити нема чого: обіцянок у ній нема, тож закриється фактом merge (§6.5): факту merge не видно — стовбура тут не знайти, або джерела кажуть про нього різне; назви стовбур у keel.toml (trunk = "..."), і факт стане видно
status-wave-plan = { "  " }{ $wave } — затверджена, ще не почата (§6.5)
status-wave-progress = { "  " }{ $wave } — в роботі; нестачі поіменно:
...
next-tag-line = { "  " }тег у тесті: { $mark } proves: { $scenario }@{ $rev }
...
next-step-commit = крок: трансформа "{ $name }" зроблена в усіх названих файлах, а коміту під її слагом на гілці нема — закоммить її роботу як `{ $name }: <слова>` (§6.2; кілька комітів дозволено, §2.4); хук пустить лише зелений (§8.4)
...
next-step-review-empty = крок: файл звіту рецензії keel/reviews/{ $wave }.md є, але він порожній — порожній файл не рецензія (§9.9): збери пакет командою `keel review` свіжому агентові, поклади його звіт у цей файл і закоммить темою chore цієї хвилі (`<слаг chore>: …`) або темою без слага
next-step-review = крок: хвиля зібрана — час рецензії (§9.9): збери пакет командою `keel review` свіжому агентові; звіт ляже в keel/reviews/{ $wave }.md і їде в історію коммітом — темою chore цієї хвилі (`<слаг chore>: …`) або темою без слага; `review: …` хук прочитає як одрук у слагу (§8.4)
next-step-light-contract-chores = крок: гілка хвилі { $wave } змінює контракт "{ $contract }", а хвиля його не називає: назви його у files трансформи — тоді хвиля повна (§6.8, §2.11: контракт варт двох поглядів), план окремо, робота окремо; або прибери зміну з гілки
...
next-step-light-contract = крок: гілка хвилі { $wave } змінює контракт "{ $contract }", а хвиля легка — контракт змінює лише повна хвиля (§6.8, §5.7): назви його у files трансформи (вага стане повною, §8.1) або прибери зміну з гілки
...
next-step-pr = крок: звіт рецензії в історії гілки — час PR, злитого кнопкою merge commit (§8.7); останнє слово про нестачі — за keel close
next-plan-branch = крок: це план-гілка хвилі { $wave } (§8.3) — доведи повноту плану (keel check, мапа), тоді дай план свіжому оку: `keel review` тут збирає пакет ПЛАНУ і каже, які відповіді перечитати, а звіт лягає в keel/reviews/{ $wave }-plan.md (§9.9); далі зливай план-PR, а робота поїде гілкою "{ $wave }"
...
init-config-header = keel.toml — налаштування keel у цьому проєкті; розкоментуй рядок, щоб увімкнути; типові значення лишаються словам keel (звідки цей словник: keel method §2.9)
init-ignore-missing = правила ignore: git не ігнорує { $path } — те, що лишає бігун цієї мови; допиши в .gitignore рівно цей рядок: { $rule } (рама радить; файлів самого проєкту вона не пише)
init-ignore-stands = правила ignore: { $path } — те, що лишає бігун, — ігнорується: правило дає { $source }, і воно їде з репозиторієм
init-ignore-exclude-only = правила ignore: { $path } — те, що лишає бігун, — ігнорує лише { $source }, а він з репозиторієм не їде: допиши в .gitignore рівно цей рядок: { $rule }
init-ignore-no-crate = правила ignore: адаптер не знайшов крейта, щоб назвати, що лишає його бігун ({ $error })
init-ignore-no-adapter = правила ignore: адаптера цього релізу в keel.toml не названо, тож і сказати, що лишає бігун, нема кому
init-ignore-leaves-nothing = правила ignore: бігун цієї мови не лишає в дереві нічого свого — ігнорувати нема чого
init-ignore-unknown-adapter = правила ignore: адаптер названий — "{ $name }", — і цей реліз його не веде: що лишає його бігун, скаже його хвиля
...
version-pin-hand = { "  " }взяти саме її: sh install.sh { $pin } — або curl -fsSL { $installer } | sh -s -- { $pin }
version-pin-road-release = { "  " }дорога цього піна: опублікований реліз, його архів і .sha256 — без git і без cargo (те саме робить KEEL_REF={ $pin } sh install.sh: дорогу вибирає форма числа, а не змінна)
version-pin-road-ref = { "  " }дорога цього піна: git ref (гілка, теґ або коміт) — клон і збірка з джерела, checksum там не звіряє ніхто; форми версії цей пін не має, тож релізу під нього не буває
version-pin-hand-border = { "  " }межа: теґа з таким іменем може не бути — тоді install.sh відмовляє переліком тих, що є; теґи старого покоління (v0.8.11 і нижче) не збираються зовсім, бо крейт тоді жив поза tool/; версія без релізу і без свого теґа збирається з гілки, якою веде remote, і лише коли зібране відповідає саме цим числом
...
ask-version = закріпити версію keel, якою судиться цей проєкт?
ask-ci = яку команду жене CI? (порожньо — пропустити)
ask-trust = записати довіру до цієї команди зараз? (§7.16 — інакше ворота відмовлять їй на першому бігу)
...
word-lang-en = англійською
word-lang-uk = українською
check-cuts-row = словник цього бінарника keel, { $lang } (сорок розрізів і їхні питання)
speak-concept-title = keel concept — поняття, на яке спирається проєкт (NEW-CONCEPT.md)
speak-concept-source = текст — NEW-CONCEPT.md, яким він був у мить ЗБІРКИ keel { $version }: знімок, зашитий у цей бінарник, а не файл із твоєї теки. Цей документ існує лише українською — англійського двійника в нього нема, і машина його не вигадує
speak-cuts-title = keel cuts — сорок розрізів якости, тими самими іменами, якими судять суди (§10.1)
speak-cuts-source = кожне питання вище — це чеклист QUALITY.md, яким він був у мить ЗБІРКИ keel { $version }: знімок, зашитий у цей бінарник, а не файл із твоєї теки; новіший чеклист потребує новішого keel. Слаги — той самий словник, яким keel check судить повноту плану (§10.3)
...
speak-cuts-headings = чеклист несе заголовків: { $read }, а суди групують розрізи під { $judged }
speak-cuts-headings-instead = девʼять родин — девʼять заголовків, у тому порядку, у якому їх тримає словник; список, який читають, згрупований інакше за той, який судять, — це знову два списки
...
check-method-row = методика цього бінарника keel, { $lang } (розділи, шматки і їхні номери)
check-method-holds = обидва тексти несуть один кістяк: ті самі розділи, ті самі номери, жодного порожнього
speak-method-empty-chapter = розділ «{ $chapter }» методики мовою { $lang } не має жодного шматка
speak-method-count = розділ { $at } несе шматків: { $judged } у { $other } і { $read } у { $lang }
speak-method-stale = англійська методика записала translated_from: { $recorded }, а український текст зараз дає { $standing }
speak-method-stale-instead = оригінал зрушив, а переклад ні: прочитай, що змінилось, приведи англійський текст і запиши нову редакцію — а якщо зміна була лише переформатуванням, запиши її і скажи це
speak-method-unrecorded = англійська методика не записала жодної редакції translated_from
speak-method-unrecorded-instead = переклад спирається на свій оригінал сильніше за будь-що тут, а хто спирається на текст — тримає його редакцію (конституція, п. 4)
speak-method-hollow = параграф §{ $number } методики мовою { $lang } не має тіла зовсім
speak-method-hollow-instead = параграф, що втратив текст, — це переклад, спинений на півдорозі: напиши його або скасуй параграф так, як каже §1.5
speak-method-chapters = методика мовою { $lang } несе розділів: { $read }, а мовою { $other } — { $judged }
speak-method-numbers = методики мовами { $other } і { $lang } нумерують параграфи по-різному: §{ $first } у { $other } проти §{ $second } у { $lang }
speak-method-skeleton-instead = одна методика — один кістяк: ті самі розділи в тому самому порядку і ті самі номери параграфів; номер, що означає різне двома мовами, не цитує нічого
speak-method-title = keel method — методика цього покоління, розділ за розділом (праворуч — скільки параграфів)
speak-method-source = текст — методика, якою вона була в мить ЗБІРКИ keel { $version }: знімок, зашитий у цей бінарник, а не файл із твоєї теки. Цей текст — джерело правди; англійський переклад іде за ним. Один шматок: keel method §8.6
...
# The briefing the tool hands a reviewer (wave 0032). Until this
# wave it lived only in a chat, written by hand for each reviewer --
# so each got a different one, and the reviewer who destroyed 10,128
# directories belonging to other sessions simply did not have the
# prohibition in his.
briefing-header = ── ДОРУЧЕННЯ РЕЦЕНЗЕНТОВІ (§9.9) ──
briefing-forbidden =
    ЧОГО НЕ РОБИТИ (спершу заборони):
      • ЧУЖОГО НЕ ЧІПАЙ. Диск і /tmp спільні. Не видаляй нічого, крім
        того, що створив сам. Ціна названа числом: рецензент хвилі
        0026 знищив 10128 чужих тек, бо цього рядка в його дорученні
        не було.
      • У репозиторій автора НЕ ПИШИ. Мутації, контрфакти і власні
        проби живуть у твоєму клоні і зникають разом із ним.
      • Не вір текстові хвилі. Число, не підтверджене твоїм бігом, —
        не число.
briefing-hygiene =
    ГІГІЄНА (без цього рецензія не рахується):
      • власний клон: git clone --no-local <корінь> <свій-шлях>;
        перевір git rev-parse --is-shallow-repository = false;
      • власний бінарник саме з тієї гілки: cd tool && cargo build
        (не встановлений keel — він старший і словник у нього інший);
      • і ДРУГИЙ бінарник із бази (main) для контрасту: без нього
        «регресія» і «давня вада» не розрізняються — половина важких
        знахідок 0034 і 0036 зміряна саме так;
      • перезбирай після мутації файлу, вшитого include_str! —
        відкат файлу не відкочує бінарника;
      • власний CARGO_TARGET_DIR — інакше ти міряєш чужий кеш;
      • місця треба мати: два target-и ≈ 3,4 ГіБ плюс keel close;
      • комітити у ВЛАСНОМУ клоні можна і треба — це твоя пісочниця;
      • червона проба лишає свою пісочницю навмисно: дивись у неї,
        перш ніж прибирати;
      • наприкінці прибери СВОЄ: клон, target (це гігабайти), теки.
briefing-work =
    ЩО ЗРОБИТИ:
      • прочитай хвилю цілою і повтори КОЖЕН мір, який автор заявив;
      • зіграй контрфакти: зніми по одному асерту або рядку суду і
        подивись, чи батарея це ЛОВИТЬ. Те, чого не ловить ніщо, —
        обіцянка, а не суд;
      • перевір, що НАЯВНЕ не зламано: батарея кілька бігів поспіль,
        cargo test -- --list проти main (чи не зник тест), clippy,
        fmt, keel check і keel rev на самому репозиторії;
      • пошукай хибні спрацювання там, де автор не дивився: чужа
        секція в конфізі, зіпсований файл, порожній репозиторій,
        повторний запуск, інша мова проєкту;
      • зіграй КОЖНУ межу, названу в тілі сценарію, окремим
        контрфактом — тіла сценаріїв вище, і кожне «тоді …» є
        зобовʼязанням, яке треба перевірити нарізно;
      • перевір, чи кожен асерт УЗАГАЛІ може впасти: суд, що ловить
        слова, яких інструмент більше не каже, зелений назавжди;
      • перевір, що жоден НАЯВНИЙ суд не звузили: дивись diff проб
        окремо від diff коду;
      • повтори умови, в яких батарею жене CI цього проєкту;
      • читай keel/reviews/ — попередні рецензії кажуть, де цей
        автор уже помилявся;
      • випиши ОКРЕМИМ БЛОКОМ усі зобовʼязання «тоді …» з тіл
        сценаріїв і відповідай по кожному: виконано / не виконано /
        виконано інакше, ніж написано;
      • випиши ОКРЕМО кожне число, яке автор назвав (скільки тестів,
        скільки знахідок, скільки бігів), і повтори кожне своїм
        міром: число, якого ніхто не перевірив, — це теж обіцянка;
      • назви межі, яких хвиля не назвала.
briefing-questions =
    ЧОТИРИ ПИТАННЯ (§9.9, тими самими словами, якими їх ставить
    методика):
      1. Про що ми змовчали?
      2. Чи всі можливі сценарії враховані?
      3. Чи реалізовано все обіцяне, без тихого звуження?
      4. Чи покриває тест весь сценарій, а не його кут?
briefing-report =
    ЗВІТ ти НЕ пишеш у репозиторій автора — ти віддаєш його текстом
    тому, хто тебе покликав, готовим лягти у keel/reviews/{ $wave }.md
    (кладе його автор; без цього файлу keel close тримає хвилю
    відкритою). Мова звіту — мова методики проєкту. Він містить:
    дату, хто, ВИРОК (прийняти / прийняти з зауваженнями / повернути
    на доробку); «Жива механіка» з числами; знахідки R-1, R-2, … з
    вагою і ДОКАЗОМ; «Не знайшов»; чотири питання стисло; власні
    промахи. Вага: ВАЖКА — обіцянку хвилі не виконано або суд
    порожній; СЕРЕДНЯ — працює, але бреше про себе або хибно
    спрацьовує на куті; ЛЕГКА — слово, число, охайність. Числа —
    ТІЛЬКИ з бігів; чого не зміг — так і напиши «не зміряв» і чому.
adapter-python-failed = pytest не запустився: { $error }
adapter-python-failed-instead = постав pytest на PATH — адаптер кличе pytest так само, як людина в терміналі
adapter-python-broken = збір тестів зламався: { $error }
adapter-python-broken-instead = pytest каже це кодом виходу 2, падіння — 1, «такого тесту нема» — 4; без збору вироку нема нікому — полагодь і повтори
adapter-python-red-unseen = pytest вийшов із { $code }, а читач не побачив жодного червоного вироку — падіння сталося поза тестами (хук сесії, teardown, плагін)
adapter-python-red-unseen-instead = жени `pytest -vv` рукою і подивись, що впало поза тестами; без прочитаного червоного суд не судить
limit-python-border = зміряно: ця мова розрізняє пʼять станів кодом виходу (0 зелене, 1 падіння, 2 злам збору, 4 такого тесту нема, 5 нічого не зібрано), тож межа §7.12 про нерозрізненність тут не стоїть — злам збору береться зламом, а невідомий вузол — «не бігло», не зеленим
limit-python-reads = не перевірено: адаптер читає лише tests/**/test_*.py і *_test.py — pytest збирає їх де завгодно під rootdir, а unittest без pytest не читається зовсім; RSpec-подібна межа, названа тут
limit-python-unread = не перевірено: { $file } адаптер не читає — pytest збирає test_*.py і *_test.py, і тег звідти не прочитано
adapter-javascript-failed = node не запустився: { $error }
adapter-javascript-failed-instead = постав node 22+ на PATH — адаптер кличе `node --test` так само, як людина в терміналі
adapter-javascript-broken = файл не завантажився: { $error }
adapter-javascript-broken-instead = node каже це тим самим кодом виходу, що й падіння, — тому суд читає TAP; без завантаження вироку нема нікому — полагодь і повтори
limit-javascript-border = не перевірено кодом виходу: node не відрізняє «впав» від «не завантажився» — обидва 1, — а імʼя без збігу дає 0 і рахує сам файл за пройдений тест; тож вирок читається з TAP (`ok`/`not ok` з іменем тесту), і код не питається ніколи
limit-javascript-reads = не перевірено: адаптер читає лише test/** і tests/** з іменами *.test.js / .mjs / .cjs / .ts / .mts — node збирає ширше (*-test.*, *_test.*, test-*.*, будь-що під test/); два тести з одним іменем у різних describe не розрізнити, бо TAP і --test-name-pattern знають лише голе імʼя — тож читаються всі рядки з цим іменем, і червоне серед них — червоне; підтест t.test тег не тримає (тег — на батьківський test); jest/vitest/mocha не читаються
limit-javascript-unread = не перевірено: { $file } адаптер не читає — тести звуться *.test.js (чи .ts), і тег звідти не прочитано
close-said-stream = { "    " }— що сказала команда ({ $stream }), дослівно і без маскування:
close-said-cut = … показано { $shown }, зрізано з середини { $count } …
close-said-shown = … показано { $count } — це весь вивід …
close-said-long = … +{ $count } знаків у цьому рядку зрізано …
close-said-not-utf8 = … вивід не був цілком UTF-8: невалідні байти замінено знаком заміни …
close-said-report-cut = { "  " }… звіт задовгий: { $count } рядків цитат зрізано …
