...
check-trunk-refused = стовбур: { $trunk } — узято за іменем. git називає { $refused }, а гілка роботи стовбуром не буває (§8.2): цей клон знято з РОБОЧОГО ДЕРЕВА, і `git clone` копіює HEAD джерела в refs/remotes/<віддалений>/HEAD. Якщо взяте вище імʼя — не та гілка, сказати це тут може лише ключ: назви стовбур у keel.toml (trunk = "...", кореневий ключ, ДО [trust] і [generated])
...
close-awaiting-merge = { $wave }: легка — самі chore, закриється фактом merge (§6.5): файла хвилі в main ще нема, merge і є закриття
...
status-wave-light-own = { "  " }{ $wave } — доводити нема чого: обіцянок у ній нема, тож закриється фактом merge (§6.5): файла хвилі в main ще нема
