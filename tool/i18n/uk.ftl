...
gate-adapter-refuses = адаптер "{ $name }" не цього релізу (реліз веде { $known }) — народження чи роботу трансформи цей суд судити не може, а суд, який не може судити, не пускає (§7.12): постав реліз, що веде цей адаптер, або назви адаптер із переліку
