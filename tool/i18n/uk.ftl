...
adapter-rails-shape = вирок розбито: у "{ $file }" рядок вироку розірвано надвоє — ім'я лишилось на одному рядку, час і мітка почали наступний. Rails сам вантажить застосунок і веде біг, тож переліку імен, який keel бере на звичайній дорозі, тут нема, і зібрати вирок нема з чого. З цього файлу не взято жодного вироку
adapter-rails-shape-instead = прожени файл рукою і подивись на вивід — на цій дорозі вирок читається з форми рядка, і розбити її може або друк із тіла тесту, або parallelize_me! (Rails вмикає паралель сам), під яким minitest фізично перемішує рядки двох тестів
