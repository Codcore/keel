...
limit-javascript-reads = не перевірено: адаптер читає лише test/** і tests/** з іменами *.test.js / .mjs / .cjs / .ts / .mts — node збирає ширше (*-test.*, *_test.*, test-*.*, будь-що під test/); два тести з одним іменем у різних describe не розрізнити, бо TAP і --test-name-pattern знають лише голе імʼя; jest/vitest/mocha не читаються
