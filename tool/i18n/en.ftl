...
check-number-twice = the number { $number } is held by two waves on disk: { $files } -- one number, one wave (§8.8)
...
check-chores-heavy-withdraws = it withdraws a scenario
...
close-awaiting-merge-unseen = { $wave }: light -- chores only, will close by the fact of merge (§6.5): there is no main here, and the court cannot see the fact
...
status-wave-light-unseen = { "  " }{ $wave } -- nothing to prove: it carries no promise, so merging will close it (§6.5): there is no main here, and the fact cannot be seen
