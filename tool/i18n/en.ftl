...
adapter-rails-shape = a verdict was cut in two: in "{ $file }" the name stayed on one line and the timing and mark began the next. Rails boots the application and owns the run, so the roll keel takes on the plain road is not available here, and there is nothing left to piece a verdict out of. No verdict was taken from this file
adapter-rails-shape-instead = run the file by hand and look at the output -- on this road a verdict is read from the shape of a line, and two things break that shape: a print from inside a test, and parallelize_me! (Rails turns parallelism on by itself), under which minitest physically interleaves the lines of two tests
