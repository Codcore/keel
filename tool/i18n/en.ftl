...
check-trunk-refused = trunk: { $trunk } -- taken without git's answer. git names { $refused }, which is a branch of the WORK -- a plan, a spike, or a branch carrying its own wave file (§8.2, §4.13) -- and a branch of the work is never a trunk. A clone taken from a working tree names one that way. If the trunk above is the wrong branch, name it in keel.toml (trunk = "...", a ROOT key, before [trust] and [generated])
check-trunk-refused-alone = no trunk could be found. git names { $refused }, which is a branch of the WORK -- a plan, a spike, or a branch carrying its own wave file (§8.2, §4.13) -- and a branch of the work is never a trunk. A clone taken from a working tree names one that way. Name the trunk in keel.toml (trunk = "...", a ROOT key, before [trust] and [generated])
