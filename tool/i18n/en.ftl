...
adapter-ruby-roll-lost = the roll is broken: in "{ $file }" this reader counted places where a verdict did not come together -- { $count }. That happens when a test prints a line shaped like a timing and a mark: the test can then close ITSELF green while its real verdict goes out with the rubbish, and every count still agrees. A green got that way is worse than a red (§4.10)
