...
close-said-outside = — and what the run said outside any one target: cargo covers every target in one process, and they write into its stderr in turn, so this text belongs to no file. cargo's own banner is cut from here -- `Compiling`, `Running`, the roll-up of failed targets: that is its bookkeeping, not words of a failure. What is left was written by somebody else, most often a test's child process:
