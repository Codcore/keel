...
adapter-ruby-unmarked = minitest's report has no end: in "{ $file }" minitest spoke its summary, and the mark keel closes that report with is nowhere in the output. The mark is printed by a `Minitest.after_run` hook keel registers LAST and which therefore runs FIRST, the instant the report is written; without it there is no telling where minitest's words end and somebody else's begin
adapter-ruby-unmarked-instead = something between minitest and keel is rewriting the stream -- a reporter of the project's own, a plugin, or an `at_exit` that replaces STDOUT and does not put it back. Run the file by hand and see whether the last line arrives; keel will not go on reading a stream like that
