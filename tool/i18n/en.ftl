...
adapter-elixir-not-utf8 = the elixir runner did not print its test names as text: the output carries replacement characters or escape sequences in place of letters, so a name cannot be matched against the one in the file -- and a court that read it would say the battery skipped a test it ran
adapter-elixir-not-utf8-instead = let the runner print unicode: unset ERL_FLAGS=+pc latin1, or give the run a UTF-8 locale (LANG=en_US.UTF-8). keel appends `+pc unicode` itself, but an explicit latin1 beats it
