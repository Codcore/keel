...
tags-not-utf8 = the file does not read as UTF-8 text: the first { $at } bytes read, and then a byte UTF-8 has no place for (the file is most likely latin-1 or cp1251)
tags-not-utf8-instead = re-encode the file as UTF-8 (`iconv -f latin1 -t utf8`) -- this court reads sources as UTF-8 and nothing else
...
graph-crossing = the scope line "{ $file }" is declared by two open waves -- { $first } and { $second } -- with no depends_on edge between them: their scopes cross, and a crossing between independent waves is a question at planning time (§8.8)
graph-crossing-instead = name a depends_on in one of the waves (then they run one after the other) or divide the file between them at planning time (§8.8)
...
graph-no-reason = an answer with no reason: { $cuts } -- the formula and nothing else; §10.3 asks for the reason: "does not apply, BECAUSE…" or "we deliberately do not, BECAUSE…"
graph-no-reason-instead = add the reason to each cut named -- one sentence on WHY that cut does not play here; closed waves are not touched, history is not rewritten
...
scope-outside = the declared file "{ $file }" lies outside the project's tree -- scope names the files of THIS tree (§4.1)
scope-outside-instead = write the path from the project's root without "..", or, if the file really is another tree's, it is not a file of this wave
...
review-plan-title = keel review -- the PLAN package (§9.9 before the merge of §6.6)
review-plan-why = the forty answers are written here, and a fresh eye reads them NOW -- while the plan can still be refused as a plan (§6.6). The machine catches a MISSING answer; only a reader catches an untrue one, and that reader's report goes to keel/reviews/{ $wave }-plan.md
review-plan-covered-header = cuts closed by a promise -- does that promise really prove THIS cut?
review-plan-covered = "{ $scenario }" claims to close: { $cuts }
review-plan-covered-body = { "    " }it promises: { $body }
review-plan-body-empty = (this promise has no section in the wave body -- there is nothing to read, and that is a finding already)
review-plan-more = { "  " }…and { $count } more promises -- read the rest in the wave file itself
review-plan-silent = cuts with NO answer: { $cuts } -- keel check already reddens over these (§10.3); until they are answered, reading the rest is early
review-plan-unknown = the branch "{ $branch }" is named as a plan, but the slug after plan/ names no wave that reads
review-plan-unknown-instead = check the branch name against the file in keel/waves/ -- plan and work carry the same name, differing only by the plan/ prefix (§8.2)
review-plan-decided-header = cuts decided without a reason -- a reason, or a shrug?
...
review-plan-footer = every question above is "yes or no, and why". A person or a fresh agent answers; the tool only says where to look. The report of this reading is keel/reviews/{ $wave }-plan.md, and without it the plan PR does not merge (§9.9)
...
adapter-ruby-roll = the roll does not match the run: minitest says it ran { $said } test(s) in "{ $file }", and the roll of names taken when the file was loaded comes to { $read }. The roll is printed on the same stream the file writes to while it loads, so a name can be added to it -- but minitest cannot be made to run that name. The count is minitest's own, and the difference is said aloud rather than guessed
adapter-ruby-roll-instead = run the file by hand and see which names stand in the roll beyond the ones that ran; a line shaped like `KEEL-ROLL Class#test_x`, printed while the file loads, makes exactly this difference
adapter-rails-shape = no verdict could be assembled: in "{ $file }" the `-v` lines do not give a name and a mark for every declared test. Rails boots the application and owns the run, so the roll keel takes on the plain road is not available here, and the shape of the line is all there is to read. No verdict was taken from this file
adapter-rails-shape-instead = run the file by hand and look at the output. Three things break that shape, and all three are measured: a print from inside a test (which cuts the verdict line in two); parallelize_me!, and Rails turns parallelism on by itself (minitest physically interleaves the lines of two tests); and minitest's spec style, describe/it, whose names carry spaces. The first is cured by a quiet test, the second by turning parallelism off, the third not yet: this road has no roll of its own, and BACKLOG carries the line
adapter-ruby-roll-none = there is no roll: minitest says it ran { $said } test(s) in "{ $file }", and the listing keel prints the moment the file is loaded is nowhere in the output
adapter-ruby-roll-none-instead = the file replaces STDOUT while it loads -- the ordinary way to quieten a noisy boot -- and swallows the roll with it; put the stream back earlier, or do not muffle it at all. Verdicts will not be read from the shape of a line here: that shape is exactly what a test forges
adapter-ruby-unsaid = the roll and the runner disagree about the names: the roll in "{ $file }" names "{ $name }", and the runner's voice never mentions it. Under `-v` minitest writes the name of every test that ran; a verdict for a name the runner never said is a verdict nobody gave
adapter-ruby-unsaid-instead = run the file by hand with `-v` and look for the name; where it is absent, the run is not the roll's, and the green here is proved by nobody
adapter-ruby-mute = ruby left with a failure and said nothing keel could read: in "{ $file }" there is no minitest summary and no verdict at all, and the exit code is not zero
adapter-ruby-mute-instead = a file that muffles STDOUT while it loads and never puts it back does this, and so does one that raises after declaring its tests. Run it by hand: a red exit is a red exit, and nought tests here would be a quiet untruth
adapter-ruby-report = minitest's report does not agree with itself: in "{ $file }" none of the summary lines in the runner's voice agrees with the named blocks -- the last summary counts { $counted } failure(s) and { $skips } skip(s), while the named blocks come to { $blocks } and { $blockskips }. The verdicts are taken from the blocks minitest writes AFTER the run, the numbers from its own summary, and a difference means one of the two was not written by minitest
adapter-ruby-report-instead = run the file by hand and look at the `1) Failure:` blocks under the summary -- printing one of those from inside a test makes exactly this difference
adapter-ruby-report-none = the report for "{ $file }" holds none of minitest's own summaries: no line shaped "N runs, N assertions, …" came out of the runner, though minitest writes one after every run. Without the summary the blocks have no count to answer to -- and blocks are exactly what a test can write
adapter-ruby-report-none-instead = run the file by hand and find the summary line; where a replaced STDOUT or a delegate of the file's own swallows it, keel will not read that output
...
adapter-rspec-printed = — and what the process itself wrote while this file ran, verbatim:
...
gate-red-green-instead = make the test truly fail; and if this is a court over your own battery or tooling, add the line `mutant: <what was broken> -> <how the probe named it>` to the message (§6.3)
gate-red-unknown = red: names "{ $slug }", not a scenario of wave { $wave }
gate-red-unknown-instead = name a scenario of this wave (`keel next` says which one is born) or write it into the wave file
gate-red-withdrawn = "{ $scenario }" is withdrawn -- a dead promise is not born (§2.12)
gate-red-withdrawn-instead = bear a live scenario, or lift `withdrawn` in the wave file if the promise is alive again
gate-red-untagged = red birth of "{ $scenario }" claimed, but no test carries its proves tag (§5.5)
gate-red-untagged-instead = put the line `proves: <scenario>@<revision>` over the test -- `keel rev` gives the revision
gate-red-many-tags = "{ $scenario }" carries { $count } proves tags -- which one is being born is not guessed
gate-red-many-tags-instead = leave one `proves` tag of this scenario, and give the rest their own scenarios
gate-red-broken = red birth of "{ $scenario }" claimed, but the tests do not compile -- a build break is not a failure (A3): { $words }
gate-red-broken-instead = fix the test build and commit again: a failure is judged only where there is something to run
gate-red-notrun = red birth of "{ $scenario }" claimed, but the run executed no test named "{ $test }" -- zero runs is not a failure (A3)
gate-red-notrun-instead = check the test's name against the tag, and that the run really executes it -- `keel next` gives the run line
...
gate-work-red-instead = carry the work through to a green test, or commit the birth `red: <scenario>` first
gate-work-stale = transform "{ $transform }": the tag of scenario "{ $scenario }" holds { $recorded }, the text gives { $actual } (§7.5)
gate-work-stale-instead = move the tags with `keel rev --write` if the scenario's text changed on purpose, or put the scenario's text back
gate-work-untagged = transform "{ $transform }": scenario "{ $scenario }" has no proves tag in the tests (§5.5)
gate-work-untagged-instead = put the line `proves: <scenario>@<revision>` over the test (`keel rev` gives the revision) and commit the birth `red: <scenario>`
gate-work-broken = transform "{ $transform }": the tests do not compile: { $words }
gate-work-broken-instead = fix the test build and commit again
gate-work-notrun = transform "{ $transform }": the run executed no test named "{ $test }" for scenario "{ $scenario }"
gate-work-notrun-instead = check the test's name against the tag, and that the run really executes it -- `keel next` gives the run line
gate-unknown-slug = "{ $slug }" is neither red: nor a transform of wave { $wave } -- a typo does not pass as "outside the judgement" (§8.4)
gate-unknown-slug-instead = name a transform slug of this wave (`keel next` says which) or write a subject that is no slug at all: then the commit goes outside the judgement (§8.4)
...
gate-case-instead = write the head of the subject in lowercase -- slugs are written lowercase (§1.2)
...
gate-adapter-refuses = the adapter "{ $name }" is not of this release (the release leads { $known }) -- a birth or a transform's work this court cannot judge, and a court that cannot judge does not pass (§7.12)
gate-adapter-refuses-instead = install a release that leads this adapter, or name one from the list in keel.toml
...
check-checked-head = what was checked:
...
check-checked-body = header-vs-body both ways (§7.7)
check-checked-close = closure is judged by keel close (§6.5)
check-unjudged = not checked: { $what }
...
close-said-outside = — and what the run said outside any one target (run { $run } of { $runs }): cargo covers every target in one process, and they write into its stderr in turn, so this text belongs to no file. cargo's own banner is cut from here -- `Compiling`, `Running <binary>`, the roll-up of failed targets: that is its bookkeeping, not words of a failure. What is left was written by somebody else, most often a test's child process:
close-test-said-run = (run { $run } of { $runs })
close-test-said-nothing = — and the runner said nothing this court could keep: under this red its voice is empty. That happens on the rspec road, where the voice is built out of JSON fields rather than kept raw: an example that falls with an anonymous exception class and an empty message leaves not one word in the document. Run the file by hand -- the process may have said more than the document took
...
close-check-blockers = findings of the documents court on this branch's own files: { $count } -- the exit is red
close-check-red-instead = instead: fix the files named above and run keel close again; `keel check` says the same thing on its own, over the whole tree
close-check-red-plan-instead = instead: nothing to do about THESE -- they say the work has not begun, and a plan PR merges as a plan (§6.6); they come back as blockers on the work branch, which is where they are answered. Whatever else this report counts stands on its own
...
close-plan-no-wave = this branch is named `plan/{ $wave }`, and no wave of that name stands in the tree (§8.2). A plan branch is approved by merging it, and there is nothing here to approve: the plan itself is missing. Code laid down on such a branch is judged by §4.9 all the same, and `keel review` refuses here with the same reason
close-lack-plan-review = the plan of wave { $wave } has met no reader: keel/reviews/{ $wave }-plan.md is not in this branch's history. §9.9 asks a fresh eye before a wave is merged, and a plan is approved by merging its own PR (§6.6) -- so this is where that eye reads. The name carries `-plan` on purpose: a report filed under the wave's own name would ride onto the work branch and satisfy the gate that exists to demand a review of the WORK; instead: `keel review` on this branch assembles the PLAN package for a fresh agent -- put the report in that file and commit it
close-lack-plan-review-empty = the plan of wave { $wave } has a report file and it is empty: keel/reviews/{ $wave }-plan.md carries nothing, and an empty file is no review (§9.9); instead: put a fresh agent's report into it and commit
...
close-lack-review = the review file keel/reviews/<wave>.md is not in the branch's history (§9.9 puts the record there, and a file no commit carries is gone from the merge); instead: commit it under a chore of this wave, or under a subject that is no slug
...
close-price = the price of this court: the battery runs three times (§7.13) into its OWN { $target } -- an inherited cache shifts verdicts (§6.7), so that is a decision, not a defect; it wants about { $needed } GiB free (measured on keel's own tree; on this tree the price is its own -- what it came to is the "price paid" line below)
...
next-step-review-empty = the step: the review file keel/reviews/{ $wave }.md exists and is empty -- an empty file is no review (§9.9): assemble the package with `keel review` for a fresh agent, put its report into this file and commit it under a chore of this wave (`<chore slug>: …`) or under a subject that is no slug at all
next-step-review = the step: the wave is assembled — time for the review (§9.9): gather the package with `keel review` for a fresh agent; the report lands at keel/reviews/{ $wave }.md and rides into history as a commit -- under a chore of this wave (`<chore slug>: …`) or under a subject that is no slug at all; `review: …` the hook reads as a typo in a slug (§8.4)
next-step-light-contract-chores = the step: the branch of wave { $wave } changes the contract "{ $contract }" and the wave does not name it: name it among the files of a transform -- the wave is then full (§6.8; §2.11: a contract is worth two human looks), plan apart and work apart; or take the change off the branch
...
next-step-pr = the step: the review report stands in the branch's history — time for the PR, merged by the merge-commit button (§8.7); the last word on lacks belongs to keel close
next-plan-branch = the step: this is the plan branch of wave { $wave } (§8.3) — prove the plan's fullness (keel check, the map), then give the plan a fresh eye: `keel review` here assembles the PLAN package and names the answers worth rereading, and the report goes to keel/reviews/{ $wave }-plan.md (§9.9); after that merge the plan PR, and the work will ride the branch "{ $wave }"
