...
review-plan-why = the forty answers are written here, and the fresh eye arrives at closing -- when the work is already done under this plan. The machine catches an ABSENT answer; an untrue one is caught only by a reader. Below is not the whole plan, but the places where untruth lives most often
...
next-plan-branch = the step: this is the plan branch of wave { $wave } (§8.3) — prove the plan's fullness (keel check, the map), merge the plan PR; the work will ride the branch "{ $wave }" -- and then give the plan a fresh eye: `keel review` here assembles the PLAN package and names the answers worth rereading before the merge (§9.9)
