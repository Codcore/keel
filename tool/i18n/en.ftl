...
close-check-red-plan = the documents court has its say: findings of `keel check` on this branch's own files (§7.1-§7.9, chapter 4): { $count }, of them counted as blockers: { $counted }. This branch's wave is approved and not started, and what the BRANCH court says there (§4.1, §4.4-§4.6, §6.2, §6.3, §7.5) is only that the work has not begun -- the state this report announces below -- so those are printed and not counted, and a plan PR merges as a plan (§6.6). Every other court counts here as anywhere
