...
close-check-red = the documents court stopped this run: { $count } findings of `keel check` (§7.1-§7.9, chapter 4) -- the battery did not run, because a court that costs six seconds answers before one that costs minutes
close-check-red-instead = instead: fix the files named above and run keel close again; `keel check` says the same thing on its own, in full
