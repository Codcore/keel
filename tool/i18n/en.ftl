...
close-check-red = the documents court has its say: { $count } findings of `keel check` on this branch's own files (§7.1-§7.9, chapter 4) -- blockers here as they are there, because the court that admits a branch to a merge is not narrower than the one beside it
close-check-red-plan = the documents court has its say: { $count } findings of `keel check` on this branch's own files (§7.1-§7.9, chapter 4) -- printed and NOT counted, because this branch's wave is approved and not started: everything the scope court can say here is that the work has not begun, and a plan PR merges as a plan (§6.6). `keel check` carries the same list with an exit code of its own
