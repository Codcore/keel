...
adapter-ruby-roll-lost = a test was declared and never judged: in "{ $file }" the file itself lists { $count } test(s) the run said nothing about — { $tests }. The list is taken when the file is LOADED, before any test body runs, so nothing a test prints can add to it or take from it: a name with no verdict means its verdict was lost, swallowed by output or never printed
