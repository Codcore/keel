...
tags-spec-one-liner = the tag proves: { $scenario }@{ $rev } stands over a one-liner example `it { "{ … }" }` -- rspec names it by its place in the file, and there is no name for a tag to hold
...
limit-rspec-border = not checked: ruby's second reading -- rspec -- reads spec/**/*_spec.rb and names an example by its full description, as rspec does (groups joined by a space, none before #… and .…); one example runs by the id a `--dry-run` gives it, the verdict comes from JSON, and pending did not run; one-liner examples `it { "{ … }" }` have no name and hold no tag, nor do examples inside shared_examples; a `describe` with a non-literal argument (described_class) gives its group no name; spec/support/ and spec_helper.rb are not read; the generated CI runs minitest -- this release writes no rspec step, add it yourself
