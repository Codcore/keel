...
adapter-ruby-unmarked = minitest's report does not stand between keel's own marks: the region keel reads in "{ $file }" holds no summary of its own. That region is closed by a mark printed from inside `Minitest.run` itself, the instant the report is written -- and three things make this refusal: the mark never arrived; the mark came BEFORE the summary (something called `Minitest.run` in the middle of the run); or the summary went missing from the stream on its way out
adapter-ruby-unmarked-instead = something between minitest and keel is rewriting the stream -- a reporter of the project's own, a plugin, an `at_exit` that replaces STDOUT and does not put it back, or a test body that runs the suite itself. Run the file by hand and see whether the summary stands before the last line; keel will not go on reading a stream like that
...
adapter-ruby-report = minitest's report does not agree with itself: in "{ $file }" the summary counts { $counted } failure(s) and { $skips } skip(s), while the named blocks come to { $blocks } and { $blockskips }. The verdicts are taken from the blocks minitest writes AFTER the run, the numbers from its own summary, and a difference means one of the two was not written by minitest
