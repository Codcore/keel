...
Принципи — одна сторінка. У цьому репозиторії це `PRINCIPLES.md`; у проєкт
вони приїжджають блоком, який `init` пише в `AGENTS.md`, і саме його агент
читає на кожному старті. Цей файл — довідник, читається за потреби.
...
```
keel/keel.py             сам інструмент, копією — його кличуть усі хуки й скіли
keel/keel.json           налаштування: дві мови, режим, адаптер, команда CI
keel/waves/              хвилі — один файл на гілку
keel/contracts/          контракти — один файл на контракт
keel/QUALITY.md          розрізи якості — чекліст при написанні хвилі
...
довше за хвилю, яка її створила.
...
Експорт пишеться або імʼям з арністю, `run/3`, або цілою сигнатурою,
`run(binary(), keyword()) :: {:ok, term()}`. Коротка форма обіцяє, що функція є;
довга обіцяє ще й форму, і там, де мову можна спитати про типи — Elixir можна —
перевірка звіряє її з тим, що оголошує модуль, і на розбіжності друкує модульну
версію. Аргументи можна називати, як їх називає сам компілятор; якщо функція несе
кілька `@spec`, обіцяти можна будь-який із них. Пишіть стільки, скільки обіцяєте:
коротка форма не слабшає від того, що існує довга. Пообіцяєте форму мовою, яка
типів не тримає, — перевірка скаже це, а не пропустить.
...
`proves` не буває порожнім: сценарій показує на контракт, бо саме його він і
доводить. Отже про контракти вирішують **разом** зі сценаріями, а не після них.
Немає потрібної обіцянки — вона приходить цією ж хвилею; є — сценарій показує на
неї, і заводити другу не треба.
...
- хвиля власного scope не має — вона сума трансформ;
...
**Те, що лише готує ґрунт, їде у файлах тієї трансформи, якій цей ґрунт
потрібен, а не окремою трансформою.** Файл збірки, перша залежність, каркас
проєкту — усе це нічого не обіцяє й нічого не наближає, тож трансформи собі не
заслуговує: `gaps` відмовить їй за те, що вона не імплементує жодного сценарію,
і матиме рацію. Кладіть їх у список першої трансформи, якій без них не
скомпілюватись.

Обстановка самого Keel під scope не підпадає — ні на гілці плану, ні на робочій.
Документи в `keel/`, скіли, хуки, CI і блок в `AGENTS.md` це план і риштування, а
не робота, і `keel update` серед роботи не має вимагати оголосити наш власний
згенерований файл у чиїйсь трансформі. Гілка `plan/*` навпаки — не має чіпати
нічого, крім цього, і перевірка це каже.

Межі звіряються з гілкою: її імʼя і є імʼям хвилі. Там, де git імені не знає —
...
У шапці хвилі редакцій сценаріїв немає: текст лежить у тілі, хеш рахується з нього
...
| Сценарій | є тест із його імʼям, він зелений, редакція збігається | прогін тестів, текст хвилі |
| Контракт | редакція збігається з записаною, і обіцянка підтверджена — експортами або командою | файл, зібраний модуль, `verify` |
| Хвиля | усі трансформи закриті, усі сценарії доведені, весь гейт зелений — перевірки, план, власна команда CI проєкту, і жодної суперечності між документами | усе вище разом |

## Шість перевірок

1. Посилання ведуть кудись: кожен слаг у шапці має свій файл або свою секцію, а
   кожне посилання в тексті — свій файл, і за межами `keel/` теж.
...
**Що означає зелене — сказано тут, бо різниця не очевидна.** Зелена пʼята
означає «тест із таким імʼям існує, редакція збігається, і він проходить», а не
«обіцянку доведено»: чи справді той тест перевіряє те, що обіцяє сценарій, не
питає ніхто, і механічно спитати неможливо. Зелена шоста означає «обіцяна форма
на місці» — модуль експортує названі функції, і підписи збігаються з
оголошеними, — а не «обіцянка виконується»: контракт може обіцяти прозою, що
зламаний інструмент не валить сесію, і мати бездоганну форму, поки код робить
протилежне.

Саме в цей зазор і б'є `/keel-review` зі своїм єдиним питанням — **про що ми
змовчали**. Перевірки ловлять розбіжність між документами й фактами; рецензія —
між тим, що написано, і тим, що малось на увазі.

Перед усіма ними йде читання самих документів. Шапка, яка не розбирається, і поле
не тієї форми — на будь-якому рівні: `transforms` списком, `files` мапою, — це
помилка документа, а не порожній усталений: порожнє читається як «нічого не
оголошено», і саме воно вимикало б охорону або звинувачувало трансформу в тому,
що вона «не оголосила файлів», коли файли поруч, лише не того виду.
...
хвилі, ні окремих рішень. Обіцянка записується один раз — сценарієм; питання живе години в
...
дві хвилі» ніхто не рахував. Те, що обіцяє, стало контрактом; те, чого свідомо
...
**Номер у назві хвилі** (`0007-`) — унікальний префікс, а не порядок. Порядок
...
Він **чекліст, а не структура**: жодна хвиля не мусить мати сценарій на кожен
...
**Один прохід на хвилю**, там, де пишуться сценарії. Не на кожному рівні й не
до збіжності: рівнів більше немає, є хвилі.
...
Етап не треба памʼятати. Хук старту сесії питає інструмент, у якому стані хвиля, і
вкидає відповідь у контекст разом з іменем потрібного скіла.

**Схвалення плану нікуди не пишеться — воно виводиться.** Файл хвилі в головній
гілці означає, що людина його прочитала й пустила; `keel next` не віддає трансформ
із хвилі, якої там ще немає. Ніяких полів і ніяких рядків журналу.
