...
Сорок розрізів під девʼятьма заголовками, якими проходить кожна хвиля, і файл,
...
**Один прохід на хвилю** — там, де пишуться сценарії. Кожен розріз питається про
те, що планується, і те, що він виявляє, стає сценарієм або рішенням. Не на
кожному рівні й не до збіжності: рівнів у Keel немає, є хвилі.
...
Два повні проходи на ту саму хвилю дають майже ті самі відповіді за подвійну
...
«Резервні копії не в цій хвилі» — це відновлюваність, названа вголос. Мовчання
...
буває неправдивим із моменту, коли його написали. Хвиля обіцяла межею «повторів
...
**І третє, найдорожче: опис бібліотеки не є її кодом.** Питання не «чи воно є», а
**звідки ви це знаєте** — з коду, який виконуватиметься, чи з тексту про нього.
Три випадки за одну добу, усі на одному проєкті. Функція існувала імʼям, а
всередині була застарілою заглушкою, що друкувала попередження й вертала `:ok` —
грепнули оголошення, не прочитали тіла. Випущена версія й гілка `main` тієї самої
бібліотеки розповідали різне: реліз тримісячної давнини, а в репозиторії за
десять днів до того змінили мову пісочниці — питали один канал. І перелік
заглушених функцій узяли з документації **сусідньої** бібліотеки, яка глушила їх
сама, — тож `print` спокійно друкував у наш stdout, доки не спіймали прогоном.

Ніхто не брехав жодного разу. Опис був правдивий про те, що описував.

Дві хвилини в джерелі закривали кожен із трьох. Тому: **читайте код залежності,
на яку спираєтесь, і прогін важить більше за код.** А для бібліотеки, чий
останній реліз старший за кілька місяців, питайте ще й репозиторій — реліз каже,
що випущено, репозиторій каже, куди воно йде.
...
**Не кожен розріз стосується кожної хвилі, і вдавати протилежне — найкоротший
