...
там де адаптер уміє читати типи, що модуль оголошує рівно цей `@spec`. Пробіли
й переноси різницею не є, тип є. На розбіжності перевірка друкує те, що оголошує
модуль, у тій формі, в якій це пишуть руками, тож виправлення — це копіювання.
Функцію зовсім без `@spec` перевірка називає, а не пропускає; так само й форму,
обіцяну мовою, адаптер якої типів не читає, — сьогодні це Python.
...
читають — перегенерація ніколи не поверне процедури моделі тихцем.
