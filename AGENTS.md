...
- **Трейлер комітів.** Кожен коміт закінчується рядком
  `Co-Authored-By: <агент> <noreply@anthropic.com>` — точне ім'я
  агента задає той, хто його запускає. Трейлер не заміняє авторства:
  автор коміта — людина або агент, що його зробив.
