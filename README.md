...
**Після `init` треба зробити дві речі, і він про них каже.** Закомітити те, що
він поклав, — інакше перший же комміт плану впреться в незакомічене. І
**перезапустити сесію агента**: теку скілів агент бере на облік при старті, тож
щойно створену не побачить, а `/clear` тут не рятує — потрібен справжній
перезапуск.
