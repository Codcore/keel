...
| `keel check` | шість перевірок — повний гейт. `--fast` лишає ті, що не запускають нічого, `--no-tests` пропускає прогін, `--branch` називає гілку там, де git її не знає |
...
| `keel init` | ставить Keel у проєкт: теки, копію інструмента, довідники, `AGENTS.md`, CI, хуки |
| `keel hook <подія> --agent` | відповідає хукові агента; кличеться конфігом, не руками |
| `keel update` | оновлює копії методики в проєкті до версії інструмента, показавши різницю *(ще немає)* |
...
**Усе, що потрапило в проєкт із методики, несе її версію.** Поки `keel update`
немає, синхронізує повторний `keel init`: він переписує тільки те, що змінилось.
...
| Codex і решта | `AGENTS.md`, десять рядків із вказівниками | читається завжди |
...
знаках у переліку скілів — довша шапка просто не доїде. Опис береться в лапки: він
містить двокрапку, а без лапок це не YAML.

`keel-plan` ще й привʼязаний до `keel/steps/*.md` через `globs`, тож у Cursor
підхоплюється сам, коли редагується файл кроку.

**Ці файли породжуються командою `keel skills`, а не ведуться руками.** Тіло в
обох діалектах те саме, різна лише шапка, і тест це стереже. Три тексти, які
пишуть окремо, розходяться за місяць — це вже було зі старою методикою.

## Стан

Зроблено: шість перевірок, `new`, `plan`, `next`, `rev`, `hooks`, `skills`,
`init`, `hook`; git-хуки й CI; три скіли у двох діалектах; хуки агента для Claude
і Cursor.

Лишилось:

1. **`keel update`** — оновити копії методики в проєкті, показавши різницю, і
   не затерти мовчки те, що правили руками.
2. **Хуки для Codex** — відкладені через `apply_patch`.
3. **Звірити вступ `QUALITY.md`** із методикою: він каже «девʼять розрізів» там,
   де їх сорок під девʼятьма заголовками, і обіцяє два проходи там, де в KEEL.md
   написано один.
...
- **Чи справді Cursor зве це поле `file_path`.** Доказ непрямий, тож хук шукає
  серед кількох імен і мовчки не пропускає. Перший же запуск у Cursor скаже
  правду; тоді список імен можна звузити.
- **Чи приходить `tool_input` рядком і для вбудованих інструментів.** Для
  MCP це видно в захоплених прикладах; читач розбирає обидві форми.
