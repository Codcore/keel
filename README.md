...
**Агента треба запускати в самій теці проєкту.** Скіли беруться з теки старту
й батьківських; ті, що лежать нижче, підхоплюються аж тоді, коли агент прочитає
там якийсь файл — а до того `/keel-plan` відповідає «Unknown skill». Якщо сесія
вже відкрита, її треба перезапустити: `/clear` теку на облік не бере, а команди
на кшталт `reload-skills` не існує.
